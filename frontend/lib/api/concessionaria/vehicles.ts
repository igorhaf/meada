...
export function createVehicle(input: CreateVehicleInput): Promise<Vehicle> {
  return apiFetch<Vehicle>('/api/concessionaria/vehicles', { method: 'POST', body: JSON.stringify(input) })
}

export function updateVehicle(id: string, input: UpdateVehicleInput): Promise<Vehicle> {
  return apiFetch<Vehicle>(`/api/concessionaria/vehicles/${id}`, { method: 'PATCH', body: JSON.stringify(input) })
}
