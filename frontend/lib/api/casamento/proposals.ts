...
export function listProposals(
  opts: { status?: string; plannerId?: string; contactId?: string; page?: number; pageSize?: number } = {},
...
export function openProposal(input: OpenProposalInput): Promise<WeddingProposal> {
  return apiFetch<WeddingProposal>('/api/casamento/proposals', { method: 'POST', body: JSON.stringify(input) })
}
...
export function updateItem(proposalId: string, itemId: string, input: UpdateItemInput): Promise<WeddingProposalItem> {
...
export function deleteItem(proposalId: string, itemId: string): Promise<void> {
  return apiFetch<void>(`/api/casamento/proposals/${proposalId}/items/${itemId}`, { method: 'DELETE' })
}

// ---- Marcos de CRONOGRAMA (NÃO entram no total) ----

export function addTimelineItem(proposalId: string, input: AddTimelineInput): Promise<WeddingTimelineItem> {
  return apiFetch<WeddingTimelineItem>(`/api/casamento/proposals/${proposalId}/timeline`, {
    method: 'POST', body: JSON.stringify(input),
  })
}

export function updateTimelineItem(proposalId: string, itemId: string, input: UpdateTimelineInput): Promise<WeddingTimelineItem> {
  return apiFetch<WeddingTimelineItem>(`/api/casamento/proposals/${proposalId}/timeline/${itemId}`, {
    method: 'PATCH', body: JSON.stringify(input),
  })
}

export function deleteTimelineItem(proposalId: string, itemId: string): Promise<void> {
  return apiFetch<void>(`/api/casamento/proposals/${proposalId}/timeline/${itemId}`, { method: 'DELETE' })
}

// ---- Tarefas do CHECKLIST pré-casamento (a escapada — NÃO entram no total, ordenadas por due_date) ----

export function addChecklistTask(proposalId: string, input: AddChecklistInput): Promise<WeddingChecklistTask> {
  return apiFetch<WeddingChecklistTask>(`/api/casamento/proposals/${proposalId}/checklist`, {
    method: 'POST', body: JSON.stringify(input),
  })
}

export function updateChecklistTask(proposalId: string, taskId: string, input: UpdateChecklistInput): Promise<WeddingChecklistTask> {
  return apiFetch<WeddingChecklistTask>(`/api/casamento/proposals/${proposalId}/checklist/${taskId}`, {
    method: 'PATCH', body: JSON.stringify(input),
  })
}

export function deleteChecklistTask(proposalId: string, taskId: string): Promise<void> {
  return apiFetch<void>(`/api/casamento/proposals/${proposalId}/checklist/${taskId}`, { method: 'DELETE' })
}

export function toggleChecklistTask(proposalId: string, taskId: string, done: boolean): Promise<WeddingChecklistTask> {
  return apiFetch<WeddingChecklistTask>(`/api/casamento/proposals/${proposalId}/checklist/${taskId}/toggle`, {
