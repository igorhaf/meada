...
import type { Coupon } from '@/profiles/sushi/sushi-types'
...
export function listCoupons(): Promise<{ items: Coupon[] }> {
  return apiFetch<{ items: Coupon[] }>('/api/sushi/coupons')
}

export function createCoupon(input: CreateCouponInput): Promise<Coupon> {
  return apiFetch<Coupon>('/api/sushi/coupons', { method: 'POST', body: JSON.stringify(input) })
}
...
export function toggleCoupon(id: string): Promise<Coupon> {
  return apiFetch<Coupon>(`/api/sushi/coupons/${id}/toggle`, { method: 'PATCH' })
}

export function deleteCoupon(id: string): Promise<void> {
  return apiFetch<void>(`/api/sushi/coupons/${id}`, { method: 'DELETE' })
}
