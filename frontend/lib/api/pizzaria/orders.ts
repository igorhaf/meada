...
import type { Order, OrderStatus } from '@/profiles/pizzaria/pizzaria-types'
...
export function listOrders(
  ...
  const qs = p.toString()
  return apiFetch<OrderPage>(`/api/pizzaria/orders${qs ? `?${qs}` : ''}`)
}

export function getOrder(id: string): Promise<Order> {
  return apiFetch<Order>(`/api/pizzaria/orders/${id}`)
}
...
export function updateOrderStatus(
  ...
  return apiFetch<Order>(`/api/pizzaria/orders/${id}/status`, {
    method: 'PATCH',
