...
import type { Order, OrderStatus } from '@/profiles/moda-infantil/moda-infantil-types'
...
export function listOrders(
  ...
  const qs = p.toString()
  return apiFetch<OrderPage>(`/api/moda-infantil/orders${qs ? `?${qs}` : ''}`)
}

export function getOrder(id: string): Promise<Order> {
  return apiFetch<Order>(`/api/moda-infantil/orders/${id}`)
}
...
export function updateOrderStatus(
  ...
  return apiFetch<Order>(`/api/moda-infantil/orders/${id}/status`, {
    method: 'PATCH',
