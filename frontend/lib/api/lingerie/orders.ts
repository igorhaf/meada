...
import type { Order, OrderStatus } from '@/profiles/lingerie/lingerie-types'
...
export function listOrders(
  ...
  const qs = p.toString()
  return apiFetch<OrderPage>(`/api/lingerie/orders${qs ? `?${qs}` : ''}`)
}

export function getOrder(id: string): Promise<Order> {
  return apiFetch<Order>(`/api/lingerie/orders/${id}`)
}
...
export function updateOrderStatus(
  ...
  return apiFetch<Order>(`/api/lingerie/orders/${id}/status`, {
    method: 'PATCH',
