...
export function listProducts(
  ...
  const qs = p.toString()
  return apiFetch<{ items: Product[] }>(`/api/lingerie/products${qs ? `?${qs}` : ''}`)
}
...
export function toggleProduct(id: string, available: boolean): Promise<Product> {
  return apiFetch<Product>(`/api/lingerie/products/${id}/toggle`, {
    method: 'PATCH',
...
export function deleteProduct(id: string): Promise<void> {
  return apiFetch<void>(`/api/lingerie/products/${id}`, { method: 'DELETE' })
}
...
export function listVariants(productId: string): Promise<{ items: Variant[] }> {
  return apiFetch<{ items: Variant[] }>(`/api/lingerie/products/${productId}/variants`)
}

export function createVariant(productId: string, input: CreateVariantInput): Promise<Variant> {
  return apiFetch<Variant>(`/api/lingerie/products/${productId}/variants`, {
    method: 'POST',
...
export function updateVariant(
  ...
  return apiFetch<Variant>(`/api/lingerie/products/${productId}/variants/${variantId}`, {
    method: 'PATCH',
    body: JSON.stringify(input),
  })
}
