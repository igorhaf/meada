...
export function createCatalogItem(input: CreateCatalogItemInput): Promise<CatalogItem> {
  return apiFetch<CatalogItem>('/api/papelaria/catalog', { method: 'POST', body: JSON.stringify(input) })
}

export function updateCatalogItem(id: string, input: UpdateCatalogItemInput): Promise<CatalogItem> {
  return apiFetch<CatalogItem>(`/api/papelaria/catalog/${id}`, { method: 'PATCH', body: JSON.stringify(input) })
}
...
export function deleteOption(itemId: string, optionId: string): Promise<void> {
  return apiFetch<void>(`/api/papelaria/catalog/${itemId}/options/${optionId}`, { method: 'DELETE' })
}
