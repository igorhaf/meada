...
export function createProfessional(input: CreateProfessionalInput): Promise<Professional> {
  return apiFetch<Professional>('/api/salon/professionals', { method: 'POST', body: JSON.stringify(input) })
}

export function updateProfessional(id: string, input: UpdateProfessionalInput): Promise<Professional> {
  return apiFetch<Professional>(`/api/salon/professionals/${id}`, { method: 'PATCH', body: JSON.stringify(input) })
}
