...
export const rowSchema: { label: string; emoji: string; fields: FieldDef[] } = {
  ...
  emoji: '🟰',
  fields: [
    { key: 'bg', label: 'Fundo', type: 'select', options: [
      { value: 'none', label: 'Transparente' },
      { value: 'muted', label: 'Suave' },
      { value: 'primary', label: 'Cor primária' },
    ] },
    { key: 'paddingY', label: 'Espaçamento vertical', type: 'select', options: [
      { value: 'none', label: 'Nenhum' },
      { value: 'sm', label: 'Pequeno' },
      { value: 'md', label: 'Médio' },
      { value: 'lg', label: 'Grande' },
    ] },
    { key: 'gap', label: 'Espaço entre colunas', type: 'select', options: [
      { value: 'sm', label: 'Pequeno' },
      { value: 'md', label: 'Médio' },
      { value: 'lg', label: 'Grande' },
    ] },
    ...
    { key: 'align', label: 'Alinhamento vertical', type: 'select', options: [
...
export const columnWidthOptions = Array.from({ length: 12 }, (_, i) => {
  const n = i + 1
  const frac = n === 12 ? 'tela cheia' : n === 6 ? 'metade' : n === 4 ? '1/3' : n === 3 ? '1/4' : `${n}/12`
...
export const columnSchema: { label: string; emoji: string; fields: FieldDef[] } = {
  ...
  emoji: '▏',
  fields: [
    { key: 'width', label: 'Largura', type: 'select', options: columnWidthOptions },
  ],
}
