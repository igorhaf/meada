...
function StatusBadge({ status }: { status: LeadStatusId }) {
  const variant =
    status === 'fechado' ? 'success'
    : status === 'em_negociacao' ? 'warning'
    : status === 'novo' ? 'info'
    : 'muted'
...
type FormState = {
  vehicleId: string; paymentCondition: PaymentCondition
  customerName: string; customerPhone: string; salespersonId: string; notes: string
}
const EMPTY: FormState = {
  vehicleId: '', paymentCondition: 'avista', customerName: '', customerPhone: '', salespersonId: '', notes: '',
}
...
export default function ConcessionariaLeadsPage() {
  ...
  const statusMutation = useMutation({
    ...
    onSuccess: (updated) => {
      qc.invalidateQueries({ queryKey: ['concessionaria-leads'] })
      setDetail(updated); setLostReason(''); setStatusError(null)
    },
    onError: (e) => {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={<Button onClick={() => { setForm(EMPTY); setFormError(null); setModalOpen(true) }}>Novo lead</Button>}
      />
      ...
      {isError ? (
        ...
        <div className="divide-y divide-border rounded-lg border border-border">
          {items.map((l) => (
            <button key={l.id} onClick={() => { setDetail(l); setLostReason(''); setStatusError(null) }}
              className="flex w-full items-center justify-between gap-3 px-4 py-3 text-left transition-colors hover:bg-muted/40">
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <span className="font-medium">{l.vehicleBrand} {l.vehicleModel}{l.vehicleYear ? ` ${l.vehicleYear}` : ''}</span>
                  <StatusBadge status={l.status} />
                </div>
                <p className="text-xs text-muted-foreground">
                  {l.customerName ?? 'Cliente'} · {formatBrl(l.vehiclePriceCents)} · {paymentConditionLabel(l.paymentCondition)}
                  {l.salespersonId ? ` · ${salespersonLabel.get(l.salespersonId) ?? 'Vendedor'}` : ''}
                </p>
              </div>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between text-xs text-muted-foreground">
          <span>Página {page + 1} de {totalPages} · {total} lead(s)</span>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Novo lead" size="md">
        <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); createMutation.mutate() }}>
          <div>
            <label className="mb-1 block text-xs font-medium text-muted-foreground">Veículo de interesse</label>
            <select value={form.vehicleId} onChange={(e) => setForm((f) => ({ ...f, vehicleId: e.target.value }))} required
              ...
              {(vehicles.data?.items ?? []).map((v) => (
                <option key={v.id} value={v.id}>{v.brand} {v.model}{v.modelYear ? ` ${v.modelYear}` : ''} — {formatBrl(v.priceCents)}</option>
              ))}
            </select>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="mb-1 block text-xs font-medium text-muted-foreground">Pagamento</label>
              <select value={form.paymentCondition}
                onChange={(e) => setForm((f) => ({ ...f, paymentCondition: e.target.value as PaymentCondition }))}
                className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm">
                {PAYMENT_CONDITIONS.map((c) => <option key={c.id} value={c.id}>{c.label}</option>)}
              </select>
            </div>
            <div>
              ...
              <select value={form.salespersonId} onChange={(e) => setForm((f) => ({ ...f, salespersonId: e.target.value }))}
                ...
                {(salespeople.data?.items ?? []).map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}
              </select>
            </div>
          </div>
          <div>
      ...
      <Modal open={detail !== null} onClose={() => setDetail(null)} title="Lead" size="md">
        {detail && (
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              <span className="font-medium">{detail.vehicleBrand} {detail.vehicleModel}{detail.vehicleYear ? ` ${detail.vehicleYear}` : ''}</span>
              <StatusBadge status={detail.status} />
            </div>
            <Card>
              <dl className="grid grid-cols-2 gap-3 text-sm">
                <div><dt className="text-xs text-muted-foreground">Cliente</dt><dd>{detail.customerName ?? '—'}</dd></div>
                <div><dt className="text-xs text-muted-foreground">Telefone</dt><dd>{detail.customerPhone ?? '—'}</dd></div>
                <div><dt className="text-xs text-muted-foreground">Veículo (valor)</dt><dd>{formatBrl(detail.vehiclePriceCents)}</dd></div>
                <div><dt className="text-xs text-muted-foreground">Pagamento</dt><dd>{paymentConditionLabel(detail.paymentCondition)}</dd></div>
                <div><dt className="text-xs text-muted-foreground">Origem</dt><dd>{detail.conversationId ? 'WhatsApp' : 'Manual'}</dd></div>
                <div><dt className="text-xs text-muted-foreground">Criado em</dt><dd>{formatDateTime(detail.createdAt)}</dd></div>
                {detail.lostReason && <div className="col-span-2"><dt className="text-xs text-muted-foreground">Motivo da perda</dt><dd>{detail.lostReason}</dd></div>}
                {detail.notes && <div className="col-span-2"><dt className="text-xs text-muted-foreground">Observações</dt><dd>{detail.notes}</dd></div>}
              </dl>
            </Card>

            <div>
              <label className="mb-1 block text-xs font-medium text-muted-foreground">Vendedor responsável</label>
              <select value={detail.salespersonId ?? ''} disabled={assignMutation.isPending}
            ...
            {ALLOWED_NEXT[detail.status].length > 0 ? (
              <div className="space-y-2">
                <label className="block text-xs font-medium text-muted-foreground">Mover no funil para…</label>
