...
import { statusLabel, type CursoEnrollmentStatusId } from '@/profiles/cursos/curso-enrollment-status'
      ...
      {isError ? (
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          <div className="divide-y divide-border rounded-lg border border-border">
            {items.map((m) => (
              <button key={m.id} onClick={() => { setSelected(m); setPayForm(EMPTY_PAY); setPayError(null) }}
          ...
          <Card>
            {!selected ? (
              <p className="text-sm text-muted-foreground">Selecione uma matrícula para ver os pagamentos.</p>
            ) : (
              <div className="space-y-3">
                <Section title={`Pagamentos — ${selected.studentName}`}><span /></Section>
                {payments.isPending ? (
                  <p className="text-sm text-muted-foreground">Carregando…</p>
                ) : (
                  <>
