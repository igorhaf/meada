...
type FormState = {
  ...
  madeToOrder: boolean
  leadTimeDays: string // dias (vazio = usa o default do config)
  specs: string
}
...
/**
 * Catálogo do PapelariaBot. Itens agrupados por categoria, toggle de disponibilidade inline,
 * criação/edição via Modal, busca por nome. ESCAPADA (8.15): cada item pode ser SOB ENCOMENDA
 * (madeToOrder) com prazo próprio (leadTimeDays; vazio usa o default do config) e declarar
 * especificações (specs: gramatura, dimensões, técnica de impressão…). Modal secundário "Opções"
 * (grupos Papel/Acabamento/Cor/Tamanho) com adicionar/editar/remover/toggle — priceDeltaCents em
 * R$. Preços salvos em centavos.
 */
export default function PapelariaCatalogPage() {
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      const trimmedLead = form.leadTimeDays.trim()
      const payload = {
        name: form.name,
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['papelaria-catalog'] })
      setModalOpen(false)
      setEditing(null)
      setForm(EMPTY_FORM)
      setFormError(null)
  ...
  const toggleMutation = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ['papelaria-catalog'] }),
  })
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Catálogo"
        description="Itens da sua papelaria. A IA usa este catálogo (com as opções e os prazos) para atender os clientes."
          ...
          <div>
            <label className="flex items-center gap-2 text-sm">
              <input
                ...
                onChange={(e) => setForm((f) => ({ ...f, madeToOrder: e.target.checked }))}
              />
              Sob encomenda (precisa de prazo de produção)
            </label>
          </div>
          {form.madeToOrder && (
            <div>
      ...
      {/* Modal secundário: opções/modifiers do item (Papel/Acabamento/Cor/Tamanho) */}
      <Modal
