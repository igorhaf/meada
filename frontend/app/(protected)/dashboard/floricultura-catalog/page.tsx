...
/**
 * Cardápio do FloriculturaBot (delivery iFood-style). Itens agrupados por categoria, toggle de
 * disponibilidade inline, criação/edição via Modal, busca por nome. ESCAPADA 2: cada item tem um
 * modal secundário "Opções" (grupos como Tamanho/Adicionais) com adicionar/editar/remover/toggle —
 * o priceDeltaCents é exibido/editado em R$. Preços salvos em centavos.
 */
export default function FloriculturaCatalogPage() {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['floricultura-catalog'] })
      setModalOpen(false)
      setEditing(null)
      setForm(EMPTY_FORM)
      setFormError(null)
  ...
  const toggleMutation = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ['floricultura-catalog'] }),
  })
    ...
    <div className="space-y-6">
      ...
      {isError ? (
        ...
        <div className="space-y-8">
          {FLORICULTURA_CATEGORIES.map((cat) => {
