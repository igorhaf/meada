...
export default function CursosSettingsPage() {
  ...
  useEffect(() => {
    if (data) {
      setForm({ opensAt: hhmm(data.opensAt), closesAt: hhmm(data.closesAt), notes: data.notes ?? '' })
    }
  }, [data])
  ...
  const saveMutation = useMutation({
    mutationFn: () => {
      ...
      return updateConfig({ opensAt: form.opensAt, closesAt: form.closesAt, notes: form.notes || null })
    },
    onSuccess: () => {
          ...
          <form className="space-y-6" onSubmit={(e) => { e.preventDefault(); saveMutation.mutate() }}>
            ...
            <Section title="Observações">
              <textarea value={form.notes}
                ...
                rows={3} placeholder="Informações gerais sobre os cursos…"
                className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm" />
            </Section>
