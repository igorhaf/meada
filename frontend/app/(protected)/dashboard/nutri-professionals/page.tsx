...
export default function NutriProfessionalsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Editar nutricionista' : 'Novo nutricionista'} size="md">
        <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); saveMutation.mutate() }}>
          ...
          <div>
            ...
            <input value={form.crn} onChange={(e) => setForm((f) => ({ ...f, crn: e.target.value }))}
              className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm" />
          </div>
          <div>
