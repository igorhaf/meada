...
export default function BarberBarbersPage() {
  ...
  const deleteMutation = useMutation({
    ...
    onError: (e) => {
      if (e instanceof ApiError && e.reason === 'barber_in_use') {
        alert('Este barbeiro tem agendamentos ou tickets de fila — não pode ser excluído. Desative-o em vez disso.')
      }
    },
  })
  ...
  return (
    <div className="space-y-6">
            ...
            <div key={b.id} className="flex items-center justify-between gap-3 px-4 py-3">
              ...
              <div className="flex shrink-0 items-center gap-3">
                <label className="flex items-center gap-1 text-xs text-muted-foreground">
                  <input type="checkbox" checked={b.active} disabled={toggleMutation.isPending}
                    onChange={() => toggleMutation.mutate(b)} />
                  ativo
                </label>
                <Button variant="outline" className="h-7 px-2 text-xs" onClick={() => openEdit(b)}>Editar</Button>
                <Button variant="outline" className="h-7 px-2 text-xs"
                  disabled={deleteMutation.isPending} onClick={() => deleteMutation.mutate(b.id)}>Excluir</Button>
              </div>
            </div>
          ))}
        </div>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Editar barbeiro' : 'Novo barbeiro'} size="md">
        <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); saveMutation.mutate() }}>
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-muted-foreground">Especialidade (opcional)</label>
