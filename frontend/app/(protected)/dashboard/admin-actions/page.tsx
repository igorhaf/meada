...
export default function AdminActionsPage() {
  ...
  function targetLink(r: AdminActionRow) {
    ...
    if (r.targetType === 'company') {
      return <Link href={`/dashboard/companies/${r.targetId}`} className="hover:underline">{r.targetType} {short}</Link>
    }
    if (r.targetType === 'user') {
      return <Link href={`/dashboard/users/${r.targetId}`} className="hover:underline">{r.targetType} {short}</Link>
    }
    return <span>{r.targetType} {short}</span>
  }
  ...
  const columns: Column<AdminActionRow>[] = [
    { key: 'createdAt', header: 'Quando', render: (r) => new Date(r.createdAt).toLocaleString('pt-BR') },
    {
      ...
      header: 'Super-admin',
      render: (r) => <span className="font-mono text-xs text-muted-foreground">{r.superAdminUserId.slice(0, 8)}</span>,
    },
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Ações de admin" description="Rastro de ações do super-admin sobre a plataforma." />
      ...
      >
        ...
        <div>
          <label className="mb-1 block text-xs font-medium text-muted-foreground">Tipo de alvo</label>
        ...
        <>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between text-xs text-muted-foreground">
              <span>Página {page + 1} de {totalPages} · {total} ação(ões)</span>
