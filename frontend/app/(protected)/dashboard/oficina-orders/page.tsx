...
function StatusBadge({ status }: { status: OsStatusId }) {
  const variant =
    status === 'aprovada' || status === 'concluida' ? 'success'
    : status === 'entregue' ? 'info'
...
export default function OficinaOrdersPage() {
  ...
  const vehicles = useQuery({ queryKey: ['oficina-vehicles-all'], queryFn: () => listVehicles({ active: true }) })
  const mechanics = useQuery({ queryKey: ['oficina-mechanics-all'], queryFn: () => listMechanics({ onlyActive: true }) })
  ...
  const openMutation = useMutation({
    ...
    onError: (e) => {
      if (e instanceof ApiError && e.reason === 'inactive_vehicle') setOpenError('Esse veículo está arquivado.')
      else if (e instanceof ApiError && e.reason === 'inactive_mechanic') setOpenError('Esse mecânico está inativo.')
  ...
  const addItemMutation = useMutation({
    ...
    onError: (e) => {
      if (e instanceof ApiError && e.reason === 'order_locked') setItemError('Esta OS não aceita mais alteração de itens.')
  ...
  const statusMutation = useMutation({
    ...
    onError: (e) => {
      ...
      if (e instanceof ApiError && e.reason === 'empty_budget') setStatusError('Adicione ao menos um item antes de orçar.')
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={<Button onClick={() => { setOpenForm(EMPTY_OPEN); setOpenError(null); setOpenModal(true) }}>Nova OS</Button>}
      />
      ...
      {isError ? (
        ...
        <div className="divide-y divide-border rounded-lg border border-border">
          {items.map((os: ServiceOrder) => (
            <button key={os.id} onClick={() => setDetailId(os.id)}
              className="flex w-full items-center justify-between gap-3 px-4 py-3 text-left transition-colors hover:bg-muted/40">
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <span className="font-medium">{os.vehiclePlate}</span>
                  {os.vehicleModel && <span className="text-xs text-muted-foreground">{os.vehicleModel}</span>}
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between text-xs text-muted-foreground">
          <span>Página {page + 1} de {totalPages} · {total} OS</span>
      ...
      {/* Modal: nova OS */}
      <Modal open={openModal} onClose={() => setOpenModal(false)} title="Nova ordem de serviço" size="md">
        <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); openMutation.mutate() }}>
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select value={openForm.vehicleId} onChange={(e) => setOpenForm((f) => ({ ...f, vehicleId: e.target.value }))} required
            ...
            <div>
              <label className="mb-1 block text-xs font-medium text-muted-foreground">Mecânico (opcional)</label>
              <select value={openForm.mechanicId} onChange={(e) => setOpenForm((f) => ({ ...f, mechanicId: e.target.value }))}
                className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm">
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-muted-foreground">Queixa do cliente</label>
            <textarea value={openForm.complaint} onChange={(e) => setOpenForm((f) => ({ ...f, complaint: e.target.value }))} required
      ...
      <Modal open={detailId !== null} onClose={() => { setDetailId(null); setStatusError(null); setItemError(null) }}
        title="Ordem de serviço" size="lg">
        {detail.isPending || !o ? (
          <p className="text-sm text-muted-foreground">Carregando…</p>
        ) : (
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              ...
              {o.vehicleModel && <span className="text-xs text-muted-foreground">{o.vehicleModel}</span>}
              <StatusBadge status={o.status} />
            </div>
            <Card>
              <dl className="grid grid-cols-2 gap-3 text-sm">
                <div><dt className="text-xs text-muted-foreground">Cliente</dt><dd>{o.customerName}</dd></div>
                <div><dt className="text-xs text-muted-foreground">Telefone</dt><dd>{o.customerPhone ?? '—'}</dd></div>
                <div><dt className="text-xs text-muted-foreground">Mecânico</dt><dd>{o.mechanicName ?? '—'}</dd></div>
                <div><dt className="text-xs text-muted-foreground">Origem</dt><dd>{o.conversationId ? 'WhatsApp' : 'Manual'}</dd></div>
                <div className="col-span-2"><dt className="text-xs text-muted-foreground">Queixa</dt><dd>{o.complaint}</dd></div>
                {o.notes && <div className="col-span-2"><dt className="text-xs text-muted-foreground">Observações</dt><dd>{o.notes}</dd></div>}
              </dl>
            </Card>
              ...
              {!itemsLocked && (
                <form className="flex flex-wrap items-end gap-2 rounded-lg border border-dashed border-border p-3"
                  ...
                  <div>
                    ...
                    <select value={itemForm.kind} onChange={(e) => setItemForm((f) => ({ ...f, kind: e.target.value as OsItemKind }))}
                  ...
                  <div className="flex-1 min-w-[8rem]">
                    ...
                    <input value={itemForm.description} onChange={(e) => setItemForm((f) => ({ ...f, description: e.target.value }))} required
                      maxLength={200} className="w-full rounded-md border border-border bg-background px-2 py-1.5 text-sm" />
                  </div>
                  <div className="w-16">
