...
type FormState = { vehicleId: string; salespersonId: string; customerName: string; date: string; time: string; notes: string }
const EMPTY: FormState = { vehicleId: '', salespersonId: '', customerName: '', date: '', time: '', notes: '' }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={<Button onClick={() => { setForm(EMPTY); setFormError(null); setConflict(null); setModalOpen(true) }}>Novo test-drive</Button>}
      />
                  ...
                  <button key={t.id} onClick={() => setDetail(t)}
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="font-medium">{t.vehicleBrand} {t.vehicleModel}{t.vehicleYear ? ` ${t.vehicleYear}` : ''}</span>
                        <StatusBadge status={t.status} />
                      </div>
                      <p className="text-xs text-muted-foreground">
                        {t.customerName ?? 'Cliente'} · {formatTime(t.startAt)}–{formatTime(t.endAt)}
                      </p>
                    </div>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between text-xs text-muted-foreground">
          <span>Página {page + 1} de {totalPages} · {total} test-drive(s)</span>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Novo test-drive" size="md">
        <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); createMutation.mutate() }}>
          <div>
            <label className="mb-1 block text-xs font-medium text-muted-foreground">Veículo (disponível)</label>
            <select value={form.vehicleId} onChange={(e) => setForm((f) => ({ ...f, vehicleId: e.target.value }))} required
              ...
              {(vehicles.data?.items ?? []).map((v) => (
                <option key={v.id} value={v.id}>{v.brand} {v.model}{v.modelYear ? ` ${v.modelYear}` : ''}</option>
              ))}
            </select>
          </div>
          <div>
            <label className="mb-1 block text-xs font-medium text-muted-foreground">Vendedor</label>
            <select value={form.salespersonId} onChange={(e) => setForm((f) => ({ ...f, salespersonId: e.target.value }))} required
              className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm">
              <option value="">Selecione…</option>
          ...
          {conflict && (
            <p className="rounded-md bg-destructive/10 px-3 py-2 text-xs text-destructive">
              Ocupado{conflict.customerName ? <> por <strong>{conflict.customerName}</strong></> : ''}, das {formatTime(conflict.startAt)} às {formatTime(conflict.endAt)}.
            </p>
          )}
        ...
        {detail && (
          <div className="space-y-4">
            ...
            {ALLOWED_NEXT[detail.status].length > 0 ? (
              ...
              <p className="text-xs text-muted-foreground">Este test-drive está num status final.</p>
            )}
          </div>
        )}
      </Modal>
