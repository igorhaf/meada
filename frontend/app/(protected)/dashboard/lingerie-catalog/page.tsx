...
import {
  ...
  updateProduct,
  updateVariant,
} from '@/lib/api/lingerie/products'
import { LINGERIE_CATEGORIES, type LingerieCategoryId } from '@/profiles/lingerie/lingerie-categories'
import { LINGERIE_SIZES, type LingerieSizeId } from '@/profiles/lingerie/lingerie-size'
import { formatBrl, type Product, type Variant } from '@/profiles/lingerie/lingerie-types'
...
const EMPTY_FORM: FormState = { name: '', description: '', price: '', category: 'sutias' }

// size, color, sku, price (R$ ou vazio = herda base), stock
type VariantForm = { size: LingerieSizeId; color: string; sku: string; price: string; stock: string }
const EMPTY_VARIANT: VariantForm = { size: 'M', color: '', sku: '', price: '', stock: '0' }

/**
 * Catálogo do LingerieBot (moda íntima / varejo). Produtos agrupados por categoria, toggle de
 * disponibilidade inline, criação/edição via Modal, busca por nome. Cada produto tem um modal
 * secundário "Variantes" — a ESCAPADA desta camada: grade tamanho × cor com SKU, preço próprio
 * (vazio = herda o preço base) e ESTOQUE (stockQty) editável. Variante sem estoque é destacada.
 * Preços salvos em centavos.
 */
export default function LingerieCatalogPage() {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['lingerie-products'] })
      setModalOpen(false)
      setEditing(null)
      setForm(EMPTY_FORM)
      setFormError(null)
  ...
  const toggleMutation = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ['lingerie-products'] }),
  })
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select value={form.category}
                onChange={(e) => setForm((f) => ({ ...f, category: e.target.value as LingerieCategoryId }))}
                className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm">
                {LINGERIE_CATEGORIES.map((c) => (
...
function VariantsEditor({ product }: { product: Product }) {
  ...
  function startEdit(v: Variant) {
    setEditingId(v.id)
    setForm({
      size: (v.size as LingerieSizeId) ?? 'M',
      color: v.color,
      sku: v.sku ?? '',
    ...
    <div className="space-y-4">
      ...
      >
        <div className="w-24">
          ...
          <select value={form.size} onChange={(e) => setForm((f) => ({ ...f, size: e.target.value as LingerieSizeId }))}
            className="w-full rounded-md border border-border bg-background px-2 py-1.5 text-sm">
            {LINGERIE_SIZES.map((s) => (
        ...
        <div className="flex-1 min-w-[7rem]">
          ...
          <input value={form.color} onChange={(e) => setForm((f) => ({ ...f, color: e.target.value }))} required
            maxLength={40} placeholder="Preto, Nude…"
            className="w-full rounded-md border border-border bg-background px-2 py-1.5 text-sm" />
        </div>
        <div className="w-28">
