...
import { listOrders, updateOrderStatus } from '@/lib/api/pizzaria/orders'
import {
  KANBAN_COLUMNS,
  NEXT_STATUS,
  STATUS_LABEL,
  ...
  type OrderItem,
  type OrderStatus,
} from '@/profiles/pizzaria/pizzaria-types'

/**
 * Resumo de um item com seus sabores (meio-a-meio) e modifiers.
 * - Pizza meio-a-meio: "1× Pizza G (½ Portuguesa / ½ Quatro Queijos, borda recheada)" — junta os
 *   flavorName das frações com " / " e anexa os modifiers; o preço já vem na regra do mais caro.
 * - Item simples: "1× X-Burger (Grande, +Bacon)" (igual comida).
 */
function itemLine(it: OrderItem): string {
  const flavors = it.flavors.map((f) => `½ ${f.flavorName}`).join(' / ')
  const opts = it.options.map((o) => o.optionLabel).join(', ')
  const inner = [flavors, opts].filter(Boolean).join(', ')
  return `${it.qtd}× ${it.itemName}${inner ? ` (${inner})` : ''}`
}
...
/**
 * Kanban de pedidos do PizzariaBot (delivery). Colunas: Aguardando aceite → Em preparo → Saiu pra
 * entrega. ESCAPADA 1 (gate de aceite): em "aguardando" o lojista Aceita (→ em_preparo) ou Recusa
 ...
 * ESCAPADA meio-a-meio: itens com flavors[] mostram as frações (½ sabor / ½ sabor). Atualiza a cada 30s.
 */
export default function PizzariaOrdersPage() {
  ...
  const active = useQuery({
    queryKey: ['pizzaria-orders', 'active'],
    queryFn: () => listOrders({ pageSize: 100 }),
    refetchInterval: 30_000,
  })

  const history = useQuery({
    queryKey: ['pizzaria-orders', 'history'],
    queryFn: () => listOrders({ pageSize: 100 }),
  ...
  const statusMutation = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ['pizzaria-orders'] }),
  })
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Pedidos" description="Aceite ou recuse novos pedidos e acompanhe o preparo e a entrega." />
