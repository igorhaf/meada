...
function StatusBadge({ status }: { status: FotografiaAppointmentStatusId }) {
  const variant =
    ...
    : status === 'entregue' ? 'success'
    : status === 'realizada' ? 'info'
    : status === 'agendada' ? 'warning'
    : 'muted'
...
const EMPTY: FormState = {
  professionalId: '', packageId: '', customerName: '', customerPhone: '', date: '', time: '', notes: '',
}
...
export default function FotografiaAppointmentsPage() {
  ...
  const materialMutation = useMutation({
    ...
    onSuccess: (updated) => {
      qc.invalidateQueries({ queryKey: ['fotografia-sessions'] })
      setDetail(updated); setMaterialError(null)
    },
    onError: (e) => {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={<Button onClick={() => { setForm(EMPTY); setFormError(null); setConflict(null); setModalOpen(true) }}>Nova sessão</Button>}
      />
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between text-xs text-muted-foreground">
          <span>Página {page + 1} de {totalPages} · {total} sessão(ões)</span>
        ...
        <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); createMutation.mutate() }}>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select value={form.packageId}
                onChange={(e) => setForm((f) => ({ ...f, packageId: e.target.value }))} required
                className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm">
                <option value="">Selecione…</option>
          ...
          {conflict && (
            <p className="rounded-md bg-destructive/10 px-3 py-2 text-xs text-destructive">
              Ocupado por <strong>{conflict.customerName}</strong>, das {formatTime(conflict.startAt)} às {formatTime(conflict.endAt)}.
            </p>
          )}
          ...
          <div className="space-y-4">
            ...
            <Card>
              <dl className="grid grid-cols-2 gap-3 text-sm">
                ...
                <div><dt className="text-xs text-muted-foreground">Pacote</dt><dd>{detail.packageName}</dd></div>
                ...
                <div><dt className="text-xs text-muted-foreground">Prazo de entrega</dt><dd>{detail.deliveryDueDate ? formatDate(detail.deliveryDueDate) : '—'}</dd></div>
              </dl>
            </Card>
            ...
            <div className="space-y-3 rounded-md border border-border p-3">
              ...
              <div>
                <label className="mb-1 block text-xs font-medium text-muted-foreground">Link do material (galeria, drive…)</label>
                <input value={materialLink} onChange={(e) => setMaterialLink(e.target.value)}
                  placeholder="https://…"
                  className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm" />
              </div>
              <div>
              ...
              <div className="flex justify-end">
                <Button className="h-8 px-3 text-xs" disabled={materialMutation.isPending}
