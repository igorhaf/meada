...
import { getConfig, updateConfig } from '@/lib/api/pizzaria/config'

type FormState = { deliveryFee: string; minOrder: string } // reais

/** Configurações do PizzariaBot (delivery): taxa de entrega + valor mínimo do pedido (em R$). */
export default function PizzariaSettingsPage() {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['pizzaria-config'] })
