...
export default function ConcessionariaSalespeoplePage() {
  ...
  const deleteMutation = useMutation({
    ...
    onError: (e) => {
      if (e instanceof ApiError && e.reason === 'salesperson_in_use') {
        alert('Este vendedor tem test-drives ou leads — não pode ser excluído. Desative-o em vez disso.')
      }
    },
  })
  ...
  return (
    <div className="space-y-6">
            ...
            <div key={s.id} className="flex items-center justify-between gap-3 px-4 py-3">
              ...
              <div className="flex shrink-0 items-center gap-3">
                <label className="flex items-center gap-1 text-xs text-muted-foreground">
                  <input type="checkbox" checked={s.active} disabled={toggleMutation.isPending}
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Editar vendedor' : 'Novo vendedor'} size="md">
