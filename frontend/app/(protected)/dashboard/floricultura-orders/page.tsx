...
import { listOrders, updateOrderStatus } from '@/lib/api/floricultura/orders'
import {
  KANBAN_COLUMNS,
  NEXT_STATUS,
  STATUS_LABEL,
...
/** Resumo de um item com seus modifiers: "1× X-Burger (Grande, +Bacon)". */
...
/**
 * Kanban de pedidos do FloriculturaBot (delivery). Colunas: Aguardando aceite → Em preparo → Saiu pra
 ...
 * Atualiza a cada 30s.
 */
export default function FloriculturaOrdersPage() {
  ...
  const active = useQuery({
    queryKey: ['floricultura-orders', 'active'],
    queryFn: () => listOrders({ pageSize: 100 }),
    refetchInterval: 30_000,
  })

  const history = useQuery({
    queryKey: ['floricultura-orders', 'history'],
    queryFn: () => listOrders({ pageSize: 100 }),
  ...
  const statusMutation = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ['floricultura-orders'] }),
  })
