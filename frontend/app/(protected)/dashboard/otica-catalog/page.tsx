...
import {
  ...
  updateCatalogItem,
  updateOption,
} from '@/lib/api/otica/catalog'
import { OTICA_CATEGORIES, type OticaCategoryId } from '@/profiles/otica/otica-categories'
import { formatBrl, type CatalogItem, type CatalogOption } from '@/profiles/otica/otica-types'
...
type FormState = {
  ...
  madeToOrder: boolean
  leadTimeDays: string // dias (vazio quando à pronta-entrega)
}

const EMPTY_FORM: FormState = {
  name: '', description: '', price: '', category: 'armacoes', madeToOrder: false, leadTimeDays: '',
}
...
/**
 * Catálogo do OticaBot (FLUXO B — pedido sob receita). Itens agrupados por categoria (Armações /
 * Lentes / Acessórios), toggle de disponibilidade inline, criação/edição via Modal, busca por nome.
 * Cada item tem um modal secundário "Opções" (grupos como Tipo de lente / Tratamento) com
 * adicionar/editar/remover/toggle — o priceDeltaCents é exibido/editado em R$. Item sob encomenda
 * (madeToOrder) tem prazo de produção em dias (leadTimeDays). Preços salvos em centavos.
 */
export default function OticaCatalogPage() {
  ...
  // Modal secundário de opções: aberto pelo botão "Opções" do item.
  const [optionsItem, setOptionsItem] = useState<CatalogItem | null>(null)

  const { data, isPending, isError } = useQuery({
    queryKey: ['otica-catalog'],
    queryFn: () => listCatalog(),
  })
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      const payload = {
        ...
        leadTimeDays: form.madeToOrder && form.leadTimeDays !== '' ? Number(form.leadTimeDays) : null,
      }
      if (editing) return updateCatalogItem(editing.id, payload)
      return createCatalogItem(payload)
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['otica-catalog'] })
      setModalOpen(false)
      setEditing(null)
      setForm(EMPTY_FORM)
      setFormError(null)
  ...
  const toggleMutation = useMutation({
    mutationFn: (it: CatalogItem) => toggleCatalogItem(it.id, !it.available),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['otica-catalog'] }),
  })
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ['otica-catalog'] }),
    onError: (e) => {
      if (e instanceof ApiError && e.reason === 'catalog_item_in_use') {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Catálogo"
        description="Armações, lentes e acessórios. A IA usa este catálogo (com as opções) para atender os clientes."
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select value={form.category}
                onChange={(e) => setForm((f) => ({ ...f, category: e.target.value as OticaCategoryId }))}
                className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm">
                {OTICA_CATEGORIES.map((c) => (
          ...
          <div className="grid grid-cols-2 gap-3">
            <label className="flex items-center gap-2 text-sm">
              <input
                ...
                onChange={(e) => setForm((f) => ({
                  ...
                  leadTimeDays: e.target.checked ? f.leadTimeDays : '',
                }))}
              />
              Sob encomenda
            </label>
            <div>
              ...
              <input type="number" min="0" step="1" value={form.leadTimeDays}
                ...
                placeholder={form.madeToOrder ? 'usa o padrão se vazio' : '—'}
      ...
      {/* Modal secundário: opções/modifiers do item (Tipo de lente / Tratamento) */}
      <Modal
...
function OptionsEditor({ item }: { item: CatalogItem }) {
  const qc = useQueryClient()
  ...
  const toggleMutation = useMutation({
    mutationFn: (op: CatalogOption) => toggleOption(item.id, op.id, !op.available),
      ...
      >
        <div className="flex-1 min-w-[8rem]">
          ...
          <input value={form.groupLabel} onChange={(e) => setForm((f) => ({ ...f, groupLabel: e.target.value }))} required
            maxLength={80} placeholder="Tipo de lente, Tratamento…"
        ...
        <div className="flex-1 min-w-[8rem]">
          ...
          <input value={form.optionLabel} onChange={(e) => setForm((f) => ({ ...f, optionLabel: e.target.value }))} required
            maxLength={80} placeholder="Antirreflexo, Multifocal…"
            className="w-full rounded-md border border-border bg-background px-2 py-1.5 text-sm" />
        </div>
        <div className="w-28">
