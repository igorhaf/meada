...
const EMPTY: FormState = { contactId: '', plate: '', brand: '', model: '', year: '', color: '', mileageKm: '', notes: '' }
...
export default function OficinaVehiclesPage() {
  ...
  const { data, isPending, isError } = useQuery({
    ...
    queryFn: () => listVehicles({ active: showInactive ? undefined : true, search: search || undefined }),
  })
    ...
    mutationFn: () => {
      ...
      if (editing) {
        return updateVehicle(editing.id, {
          plate: form.plate, brand: form.brand || null, model: form.model || null,
          year, color: form.color || null, mileageKm, notes: form.notes || null,
        })
      }
      return createVehicle({
        contactId: form.contactId, plate: form.plate, brand: form.brand || null,
        model: form.model || null, year, color: form.color || null, mileageKm, notes: form.notes || null,
      })
    },
    onSuccess: () => {
  ...
  function openEdit(v: OsVehicle) {
    setEditing(v)
    setForm({
      contactId: v.contactId, plate: v.plate, brand: v.brand ?? '', model: v.model ?? '',
      year: v.year != null ? String(v.year) : '', color: v.color ?? '',
      mileageKm: v.mileageKm != null ? String(v.mileageKm) : '', notes: v.notes ?? '',
    })
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar por placa/modelo/marca…"
              ...
              <div className="flex shrink-0 items-center gap-3">
                ...
                {v.active && (
                  <Button variant="outline" className="h-7 px-2 text-xs"
                    disabled={archiveMutation.isPending} onClick={() => archiveMutation.mutate(v.id)}>Arquivar</Button>
                )}
                <Button variant="outline" className="h-7 px-2 text-xs"
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Editar veículo' : 'Novo veículo'} size="md">
        <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); saveMutation.mutate() }}>
          {!editing && (
            <div>
              <label className="mb-1 block text-xs font-medium text-muted-foreground">Cliente (contato)</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input value={form.plate} onChange={(e) => setForm((f) => ({ ...f, plate: e.target.value.toUpperCase() }))} required
