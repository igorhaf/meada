...
const EMPTY: FormState = { professionalId: '', patientId: '', appointmentType: 'primeira', date: '', time: '', notes: '' }
...
export default function NutriAppointmentsPage() {
  ...
  const professionals = useQuery({ queryKey: ['nutri-appt-professionals'], queryFn: () => listProfessionals({ onlyActive: true }) })
  const patients = useQuery({ queryKey: ['nutri-appt-patients'], queryFn: () => listPatients({ active: true }) })
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={<Button onClick={() => { setForm(EMPTY); setFormError(null); setConflict(null); setModalOpen(true) }}>Nova consulta</Button>}
      />
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between text-xs text-muted-foreground">
          <span>Página {page + 1} de {totalPages} · {total} consulta(s)</span>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Nova consulta" size="md">
        <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); createMutation.mutate() }}>
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-muted-foreground">Tipo de consulta</label>
            <select value={form.appointmentType}
              onChange={(e) => setForm((f) => ({ ...f, appointmentType: e.target.value as NutriAppointmentType }))} required
              className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm">
              {TYPES.map((t) => (
          ...
          {conflict && (
            <p className="rounded-md bg-destructive/10 px-3 py-2 text-xs text-destructive">
              Ocupado por <strong>{conflict.patientName}</strong>, das {formatTime(conflict.startAt)} às {formatTime(conflict.endAt)}.
            </p>
          )}
          ...
          <div className="space-y-4">
            ...
            <Card>
              <dl className="grid grid-cols-2 gap-3 text-sm">
                ...
                <div><dt className="text-xs text-muted-foreground">Tipo</dt><dd>{typeLabel(detail.appointmentType)}</dd></div>
                <div><dt className="text-xs text-muted-foreground">Telefone</dt><dd>{detail.patientPhone ?? '—'}</dd></div>
