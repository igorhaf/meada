...
import {
  ...
  statusLabel,
  type SalonAppointmentStatusId,
} from '@/profiles/salon/salon-appointment-status'
import {
  formatDate,
  formatTime,
  type Appointment,
  type ConflictDetail,
} from '@/profiles/salon/salon-types'
...
/**
 * Agenda do SalãoBot (camada 7.5). Lista por dia com filtro de status e de profissional, criação
 * manual via Modal (trata 409 conflict_slot por profissional + 400 inactive_*) e detalhe com
 * mudança de status (notifica cliente em confirmado/cancelado).
 */
export default function SalonAppointmentsPage() {
  ...
  const [statusTarget, setStatusTarget] = useState<SalonAppointmentStatusId | null>(null)
  ...
  const createMutation = useMutation({
    ...
    onError: (e) => {
      ...
      if (e instanceof ApiError && e.reason === 'conflict_slot') {
        setFormError('Esse profissional já tem agendamento nesse horário.')
        ...
        setFormError('Esse horário está fora do funcionamento do salão.')
  ...
  const statusMutation = useMutation({
    mutationFn: (newStatus: SalonAppointmentStatusId) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <Modal open={detail !== null} onClose={() => setDetail(null)} title="Agendamento" size="md">
        {detail && (
