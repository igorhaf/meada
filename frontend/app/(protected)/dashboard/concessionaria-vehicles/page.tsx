...
const EMPTY: FormState = {
  brand: '', model: '', modelYear: '', mileageKm: '', price: '',
  color: '', fuel: '', transmission: '', plate: '', photoUrl: '', description: '',
}
...
export default function ConcessionariaVehiclesPage() {
  ...
  const statusMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['concessionaria-vehicles'] })
      setStatusVehicle(null); setStatusTarget(null); setStatusError(null)
    },
    onError: (e) => {
  ...
  function openEdit(v: Vehicle) {
    ...
    setForm({
      ...
      color: v.color ?? '', fuel: v.fuel ?? '', transmission: v.transmission ?? '',
      plate: v.plate ?? '', photoUrl: v.photoUrl ?? '', description: v.description ?? '',
    })
  ...
  return (
    <div className="space-y-6">
            ...
            <div key={v.id} className="flex items-center justify-between gap-3 px-4 py-3">
              <div className="flex min-w-0 items-center gap-3">
                {v.photoUrl ? (
                  ...
                  <div className="flex h-12 w-16 shrink-0 items-center justify-center rounded bg-muted text-[10px] text-muted-foreground">sem foto</div>
                )}
              ...
              <div className="flex shrink-0 items-center gap-2">
                {ALLOWED_NEXT[v.status].length > 0 && (
                  <Button variant="outline" className="h-7 px-2 text-xs"
                    onClick={() => { setStatusVehicle(v); setStatusTarget(null); setStatusError(null) }}>
                    Status
                  </Button>
                )}
                <Button variant="outline" className="h-7 px-2 text-xs" onClick={() => openEdit(v)}>Editar</Button>
                <Button variant="outline" className="h-7 px-2 text-xs"
                  disabled={deleteMutation.isPending} onClick={() => deleteMutation.mutate(v.id)}>Excluir</Button>
              </div>
            </div>
          ))}
        </div>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Editar veículo' : 'Novo veículo'} size="lg">
        <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); saveMutation.mutate() }}>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <input type="number" min="1900" max="2100" value={form.modelYear}
                onChange={(e) => setForm((f) => ({ ...f, modelYear: e.target.value }))}
                className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-muted-foreground">Quilometragem</label>
              <input type="number" min="0" value={form.mileageKm}
            ...
            <div>
              ...
              <input value={form.price} onChange={(e) => setForm((f) => ({ ...f, price: e.target.value }))} required
                placeholder="0,00" className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm" />
            </div>
          </div>
          <div className="grid grid-cols-3 gap-3">
            <div>
            ...
            <div>
              <label className="mb-1 block text-xs font-medium text-muted-foreground">Combustível</label>
              <input value={form.fuel} onChange={(e) => setForm((f) => ({ ...f, fuel: e.target.value }))}
                list="conc-fuels" placeholder="Flex, Diesel…"
                className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm" />
              <datalist id="conc-fuels">{FUELS.map((x) => <option key={x} value={x} />)}</datalist>
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-muted-foreground">Câmbio</label>
              <input value={form.transmission} onChange={(e) => setForm((f) => ({ ...f, transmission: e.target.value }))}
                list="conc-transmissions" placeholder="Manual, Automático…"
                className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm" />
              <datalist id="conc-transmissions">{TRANSMISSIONS.map((x) => <option key={x} value={x} />)}</datalist>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input value={form.plate} onChange={(e) => setForm((f) => ({ ...f, plate: e.target.value.toUpperCase() }))}
                maxLength={10} className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-muted-foreground">Foto (URL)</label>
      ...
      <Modal open={statusVehicle !== null} onClose={() => setStatusVehicle(null)} title="Mudar status do veículo" size="sm">
        {statusVehicle && (
          <div className="space-y-4">
            <p className="text-sm">
              <strong>{statusVehicle.brand} {statusVehicle.model}</strong> está{' '}
              <StatusBadge status={statusVehicle.status} />
            </p>
            <div>
              <label className="mb-1 block text-xs font-medium text-muted-foreground">Mudar para…</label>
