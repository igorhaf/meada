...
import { getConfig, updateConfig } from '@/lib/api/lingerie/config'
import { useSyncedForm } from '@/lib/use-synced-form'

type FormState = { deliveryFee: string; minOrder: string } // reais

/** Configurações do LingerieBot (varejo): taxa de entrega + valor mínimo do pedido (em R$). */
export default function LingerieSettingsPage() {
  const qc = useQueryClient()
  const [error, setError] = useState<string | null>(null)
  const [saved, setSaved] = useState(false)

  const { data, isPending, isError } = useQuery({
    queryKey: ['lingerie-config'],
    queryFn: () => getConfig(),
  })
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['lingerie-config'] })
