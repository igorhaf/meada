...
import { listOrders, updateOrderStatus } from '@/lib/api/moda-infantil/orders'
import {
  FULFILLMENT_LABEL,
  KANBAN_COLUMNS,
  NEXT_STATUS,
  ...
  type OrderItem,
  type OrderStatus,
} from '@/profiles/moda-infantil/moda-infantil-types'

/** Resumo de um item com sua variante: "1× Body Bebê (3-6m / Azul)". */
...
/**
 * Kanban de pedidos do ModaInfantilBot (roupa de criança / varejo). Colunas: Aguardando aceite →
 * Separando → Enviado. Gate de aceite: em "aguardando" o lojista Aceita (→ separando) ou Recusa
 ...
 * Cada item mostra a variante escolhida ("1× Body Bebê (3-6m / Azul)") e o card exibe a forma de
 * entrega (entrega/retirada) com endereço. Atualiza a cada 30s.
 */
export default function ModaInfantilOrdersPage() {
  ...
  const active = useQuery({
    queryKey: ['moda-infantil-orders', 'active'],
    queryFn: () => listOrders({ pageSize: 100 }),
    refetchInterval: 30_000,
  })

  const history = useQuery({
    queryKey: ['moda-infantil-orders', 'history'],
    queryFn: () => listOrders({ pageSize: 100 }),
  ...
  const statusMutation = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ['moda-infantil-orders'] }),
  })
