          ...
          <form className="space-y-6" onSubmit={(e) => { e.preventDefault(); saveMutation.mutate() }}>
            <Section title="Exames (horário de funcionamento)">
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                ...
                <div>
                  <label className="mb-1 block text-xs font-medium text-muted-foreground">Duração do exame (min)</label>
                  <input type="number" min={5} step={5} value={form.examDurationMinutes}
                    onChange={(e) => setForm((f) => f && { ...f, examDurationMinutes: Number(e.target.value) })}
                    className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm" />
                </div>
              </div>
            </Section>
            ...
            <Section title="Óculos (pedidos sob receita)">
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div>
                  <label className="mb-1 block text-xs font-medium text-muted-foreground">Prazo de produção padrão (dias)</label>
                  <input type="number" min={0} step={1} value={form.leadTimeDaysDefault}
                    onChange={(e) => setForm((f) => f && { ...f, leadTimeDaysDefault: Number(e.target.value) })}
                    className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm" />
                </div>
              </div>
            </Section>
