...
const EMPTY: FormState = { contactId: '', name: '', goal: '', dietaryRestrictions: '', birthDate: '', notes: '' }
    ...
    <div className="space-y-6">
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Editar paciente' : 'Novo paciente'} size="md">
        <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); saveMutation.mutate() }}>
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-muted-foreground">Restrições alimentares</label>
