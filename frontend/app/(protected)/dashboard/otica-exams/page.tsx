...
export default function OticaExamsPage() {
  ...
  const professionals = useQuery({ queryKey: ['otica-exam-professionals'], queryFn: () => listProfessionals({ onlyActive: true }) })
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={<Button onClick={() => { setForm(EMPTY); setFormError(null); setConflict(null); setModalOpen(true) }}>Novo exame</Button>}
      />
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between text-xs text-muted-foreground">
          <span>Página {page + 1} de {totalPages} · {total} exame(s)</span>
        ...
        <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); createMutation.mutate() }}>
          <div>
            ...
            <input value={form.customerName} onChange={(e) => setForm((f) => ({ ...f, customerName: e.target.value }))} required
              maxLength={200} placeholder="Nome do cliente"
