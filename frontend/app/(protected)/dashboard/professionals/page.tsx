...
import {
  ...
  toggleProfessional,
  updateProfessional,
} from '@/lib/api/salon/professionals'
...
/**
 * Profissionais do SalãoBot (camada 7.5). Lista com toggle ativo inline, criação/edição via Modal.
 * A IA oferece profissionais ativos ao agendar; o conflito de agenda é por profissional.
 */
export default function ProfessionalsPage() {
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      const payload = { name: form.name, specialty: form.specialty || null, notes: form.notes || null }
      if (editing) return updateProfessional(editing.id, payload)
      return createProfessional(payload)
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['salon-professionals'] })
  ...
  const toggleMutation = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ['salon-professionals'] }),
  })
  ...
  const deleteMutation = useMutation({
    ...
    onError: (e) => {
      if (e instanceof ApiError && e.reason === 'professional_in_use') {
        alert('Este profissional tem agendamentos — não pode ser excluído. Desative-o em vez disso.')
      }
    },
  })
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Profissionais"
        description="Equipe do salão. A IA oferece os profissionais ativos ao agendar."
            ...
            <div key={p.id} className="flex items-center justify-between gap-3 px-4 py-3">
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {p.specialty && <Badge variant="muted">{p.specialty}</Badge>}
        ...
        <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); saveMutation.mutate() }}>
          ...
          <div>
            ...
            <input value={form.specialty} onChange={(e) => setForm((f) => ({ ...f, specialty: e.target.value }))}
              placeholder="Cabeleireira, Manicure, Esteticista…"
