...
import {
  ...
  toggleService,
  updateService,
} from '@/lib/api/salon/services'
import { formatPrice, type Offering } from '@/profiles/salon/salon-types'
...
const EMPTY: FormState = { name: '', category: '', durationMinutes: '45', price: '', description: '' }

/** Agrupa serviços por categoria (null vira "Sem categoria"). */
function groupByCategory(items: Offering[]): { category: string; items: Offering[] }[] {
...
/**
 * Serviços do SalãoBot (camada 7.5). Rota /dashboard/salon-services (a /dashboard/services é a tela
 * genérica de configuração da IA do core — não colidir). Agrupados por categoria, toggle ativo,
 * CRUD via Modal. Duração por serviço (snapshot no agendamento); preço opcional.
 */
export default function SalonServicesPage() {
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      if (editing) {
        // sentinela -1 limpa o preço; vazio mantém; valor grava.
  ...
  const toggleMutation = useMutation({
    mutationFn: (o: Offering) => toggleService(o.id, !o.active),
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Serviços"
        description="Catálogo de serviços do salão. A IA oferece estes serviços ao atender."
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input value={form.category} onChange={(e) => setForm((f) => ({ ...f, category: e.target.value }))}
                placeholder="Cabelo, Unha, Pele…"
                className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-muted-foreground">Duração (min)</label>
              <input type="number" min="15" max="480" step="15" value={form.durationMinutes} required
