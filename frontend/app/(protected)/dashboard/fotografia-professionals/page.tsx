...
import {
  ...
  toggleProfessional,
  updateProfessional,
} from '@/lib/api/fotografia/professionals'
...
/**
 * Profissionais do estúdio (camada 8.16). CRUD via Modal, toggle ativo, exclusão protegida (409
 * professional_in_use - orienta a desativar). Rota /dashboard/fotografia-professionals.
 */
export default function FotografiaProfessionalsPage() {
  const qc = useQueryClient()
  const [modalOpen, setModalOpen] = useState(false)
  const [editing, setEditing] = useState<FotografiaProfessional | null>(null)
  const [form, setForm] = useState<FormState>(EMPTY)
  const [formError, setFormError] = useState<string | null>(null)

  const { data, isPending, isError } = useQuery({
    queryKey: ['fotografia-professionals'],
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['fotografia-professionals'] })
  ...
  const toggleMutation = useMutation({
    mutationFn: (p: FotografiaProfessional) => toggleProfessional(p.id, !p.active),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['fotografia-professionals'] }),
  })
  ...
  const deleteMutation = useMutation({
    ...
    onError: (e) => {
      if (e instanceof ApiError && e.reason === 'professional_in_use') {
        alert('Este profissional tem sessões — não pode ser excluído. Desative-o.')
      }
    },
  })

  function openCreate() { setEditing(null); setForm(EMPTY); setFormError(null); setModalOpen(true) }
  function openEdit(p: FotografiaProfessional) {
    setEditing(p)
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Profissionais"
        description="Fotógrafos e cinegrafistas do estúdio. A IA agenda na agenda de cada um."
        ...
        <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); saveMutation.mutate() }}>
          ...
          <div>
            ...
            <input value={form.specialty} onChange={(e) => setForm((f) => ({ ...f, specialty: e.target.value }))}
              placeholder="Ensaio, casamento, vídeo, corporativo…"
