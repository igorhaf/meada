...
import { listOrders, updateOrderStatus } from '@/lib/api/lingerie/orders'
import {
  FULFILLMENT_LABEL,
  KANBAN_COLUMNS,
  NEXT_STATUS,
  ...
  type OrderItem,
  type OrderStatus,
} from '@/profiles/lingerie/lingerie-types'

/** Resumo de um item com sua variante: "1× Sutiã Renda (M / Preto)". */
function itemLine(it: OrderItem): string {
  return `${it.qtd}× ${it.productName} (${it.size} / ${it.color})`
}
...
/** Localização do pedido: endereço quando entrega; "Retirada na loja" quando retirada. */
function locationLine(order: Order): string {
  if (order.fulfillment === 'retirada') return 'Retirada na loja'
  return order.deliveryAddress ?? 'Endereço não informado'
}
...
/**
 * Kanban de pedidos do LingerieBot (moda íntima / varejo). Colunas: Aguardando aceite → Separando →
 * Enviado. Gate de aceite: em "aguardando" o lojista Aceita (→ separando) ou Recusa (Modal com
 * motivo OPCIONAL → status recusado). Nas demais colunas: Avançar / Cancelar (confirmado via
 * AlertDialog). Entregues/recusados/cancelados ficam na aba Histórico (recusado mostra o motivo).
 * Cada item mostra a variante escolhida ("1× Sutiã Renda (M / Preto)") e o card exibe a forma de
 * entrega (entrega/retirada) com endereço. Atualiza a cada 30s.
 */
export default function LingerieOrdersPage() {
  ...
  const active = useQuery({
    queryKey: ['lingerie-orders', 'active'],
    queryFn: () => listOrders({ pageSize: 100 }),
    refetchInterval: 30_000,
  })

  const history = useQuery({
    queryKey: ['lingerie-orders', 'history'],
    queryFn: () => listOrders({ pageSize: 100 }),
  ...
  const statusMutation = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ['lingerie-orders'] }),
  })

  function accept(o: Order) {
    statusMutation.mutate({ id: o.id, status: 'separando' })
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Pedidos" description="Aceite ou recuse novos pedidos e acompanhe a separação e o envio." />
              ...
              {historyItems.map((o) => (
                <div key={o.id} className="flex flex-col gap-1 px-4 py-3 text-sm">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <Badge variant="muted">{FULFILLMENT_LABEL[o.fulfillment]}</Badge>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              maxLength={300}
              placeholder="Ex.: variante sem estoque, fora da área de entrega…"
