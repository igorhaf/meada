...
function itemLine(it: OrderItem): string {
  ...
  return `${it.qty}× ${it.serviceName}${opts ? ` (${opts})` : ''}`
}
 ...
 * ESCAPADA (8.10): mostra as DUAS datas — coleta e entrega — + o período da coleta.
 */
function OrderCard({
  order,
  onAccept,
...
/**
 * Kanban de pedidos do LavanderiaBot. Colunas: Aguardando aceite → Coletado → Em processo → Pronto →
 * Saiu pra entrega. Gate de aceite: em "aguardando" Aceita (→ coletado) ou Recusa (Modal com motivo
 * OPCIONAL). Nas demais: Avançar / Cancelar (AlertDialog). Entregues/recusados/cancelados ficam no
 * Histórico (recusado mostra o motivo). Atualiza a cada 30s.
 */
export default function LavanderiaOrdersPage() {
  ...
  const active = useQuery({
    queryKey: ['lavanderia-orders', 'active'],
    queryFn: () => listOrders({ pageSize: 100 }),
    refetchInterval: 30_000,
  })

  const history = useQuery({
    queryKey: ['lavanderia-orders', 'history'],
    queryFn: () => listOrders({ pageSize: 100 }),
  ...
  const statusMutation = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ['lavanderia-orders'] }),
  })

  function accept(o: Order) {
    statusMutation.mutate({ id: o.id, status: 'coletado' })
  }
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              maxLength={300}
              placeholder="Ex.: fora da área de coleta, peça não atendida…"
