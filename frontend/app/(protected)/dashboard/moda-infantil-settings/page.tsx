...
import { getConfig, updateConfig } from '@/lib/api/moda-infantil/config'

type FormState = { deliveryFee: string; minOrder: string } // reais

/** Configurações do ModaInfantilBot (varejo): taxa de entrega + valor mínimo do pedido (em R$). */
export default function ModaInfantilSettingsPage() {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['moda-infantil-config'] })
