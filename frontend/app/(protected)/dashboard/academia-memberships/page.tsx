...
export default function AcademiaMembershipsPage() {
  ...
  const plans = useQuery({ queryKey: ['academia-plans-all'], queryFn: () => listPlans({ onlyActive: true }) })
  ...
  const createMutation = useMutation({
    ...
    onError: (e) => {
      ...
      } else if (e instanceof ApiError && (e.reason === 'plan_inactive' || e.reason === 'class_inactive')) {
  ...
  function toggleClassId(id: string) {
    setForm((f) => f.classIds.includes(id)
        ...
        <div className="divide-y divide-border rounded-lg border border-border">
          {items.map((m) => (
            <button key={m.id} onClick={() => setDetail(m)}
              ...
              <span className="text-xs text-muted-foreground">{formatPrice(m.planMonthlyCents)}/mês</span>
            </button>
          ))}
        </div>
      )}
          ...
          <div>
            ...
            <select value={form.planId} onChange={(e) => setForm((f) => ({ ...f, planId: e.target.value }))} required
              ...
              {(plans.data?.items ?? []).map((p) => (
                <option key={p.id} value={p.id}>{p.name} ({formatPrice(p.monthlyCents)}/mês)</option>
              ))}
            </select>
          </div>
          <div>
          ...
          <div className="space-y-4">
            ...
            <Card>
              <dl className="grid grid-cols-2 gap-3 text-sm">
                <div><dt className="text-xs text-muted-foreground">Plano</dt><dd>{detail.planName} ({formatPrice(detail.planMonthlyCents)}/mês)</dd></div>
                ...
                <div className="col-span-2">
                  ...
                  <dd>{detail.classes.map((c) => `${dayOfWeekLabel(c.dayOfWeek)} ${formatTime(c.startTime)} ${c.className}`).join(' · ') || '—'}</dd>
                </div>
              </dl>
            </Card>
                    ...
                    <ul className="space-y-1 text-sm">
                      {payments.data!.items.map((p) => (
                        <li key={p.id} className="flex items-center justify-between gap-2">
                          <span>{formatMonth(p.referenceMonth)} · {formatPrice(p.amountCents)}{p.method ? ` · ${p.method}` : ''}</span>
