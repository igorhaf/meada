...
const EMPTY_FORM: FormState = { name: '', description: '', price: '', category: 'vinhos' }
...
/**
 * Cardápio do AdegaBot (delivery de bebidas). Itens agrupados por categoria, toggle de
 * disponibilidade inline, criação/edição via Modal, busca por nome. Cada item tem um modal
 * secundário "Opções" (grupos como Volume/Temperatura) com adicionar/editar/remover/toggle —
 * o priceDeltaCents é exibido/editado em R$. Preços salvos em centavos.
 */
export default function AdegaMenuPage() {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['adega-menu'] })
      setModalOpen(false)
      setEditing(null)
      setForm(EMPTY_FORM)
      setFormError(null)
  ...
  const toggleMutation = useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ['adega-menu'] }),
  })
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Cardápio"
        description="Itens da sua adega. A IA usa este cardápio (com as opções) para atender os clientes."
      ...
      {/* Modal secundário: opções/modifiers do item (Volume / Temperatura) */}
      <Modal
