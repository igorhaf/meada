import type { ModaInfantilCategoryId } from './moda-infantil-categories'

/**
 * Tipos do perfil moda_infantil (roupa de criança / varejo, camada 8.22).
 *
 * ESCAPADA da camada (vs lingerie): o eixo de tamanho da GRADE DE VARIANTES (tamanho × cor) passa a
 * ser FAIXAS ETÁRIAS (age bands — ver kids-size.ts) em vez das letras PP..XGG. O modelo é idêntico:
 * o cliente pede uma variante específica; o item do pedido referencia a variante e congela
 * name/size/color/preço.
 */

/**
 * Variante de um produto (espelha ModaInfantilVariant do backend): combinação tamanho × cor com
...
/** Produto de catálogo (espelha ModaInfantilProduct do backend) + suas variantes. */
export type Product = {
  id: string
  name: string
...
/**
 * Pedido (espelha ModaInfantilOrder). rejectionReason preenchido só quando status = recusado.
