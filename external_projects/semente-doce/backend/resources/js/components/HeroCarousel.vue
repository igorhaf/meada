        ...
        >
            <Transition mode="out-in" enter-active-class="transition duration-500" enter-from-class="opacity-0 translate-y-3" leave-active-class="transition duration-200" leave-to-class="opacity-0">
                <div :key="current" class="max-w-xl">
                    <span class="mb-3 inline-block rounded-full bg-white/20 px-3 py-1 text-xs font-semibold uppercase tracking-wide backdrop-blur">Fresquinho da confeitaria</span>
                    ...
                    <a v-if="active.link" :href="active.link" class="mt-6 inline-flex items-center gap-2 rounded-full bg-white px-6 py-3 text-sm font-bold text-neutral-900 shadow-lg transition hover:bg-neutral-100">
