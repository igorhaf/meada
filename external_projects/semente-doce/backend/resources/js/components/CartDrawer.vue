        ...
        >
            ...
            >
                <header class="flex items-center justify-between border-b border-neutral-200 px-5 py-4">
                    <h2 class="text-lg font-bold text-neutral-900">Sua sacola 🍬</h2>
                ...
                <div v-if="cart.items.length === 0" class="flex flex-1 flex-col items-center justify-center gap-3 px-6 text-center">
                    ...
                    <p class="text-neutral-500">Sua sacola está vazia.<br />Que tal um docinho pra adoçar o dia?</p>
                    <button class="btn-outline mt-2" @click="closeDrawer">Ver o cardápio</button>
                </div>

                <div v-else class="flex-1 space-y-4 overflow-y-auto px-5 py-4">
                    <div v-for="item in cart.items" :key="item.key" class="flex gap-3">
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <p v-if="item.optionsSummary" class="line-clamp-2 text-xs text-neutral-500">{{ item.optionsSummary }}</p>
                            <p v-else class="text-xs text-neutral-400">{{ item.unit }}</p>
                            <div class="mt-1.5 flex items-center justify-between">
                                <div class="inline-flex items-center rounded-full border border-neutral-300 text-sm">
                                    <button class="px-2 py-1 text-neutral-600 hover:text-brand-700" @click="setQty(item.key, item.qty - 1)">−</button>
                        ...
                        <button class="self-start text-neutral-400 hover:text-red-500" @click="removeFromCart(item.key)" aria-label="Remover">
                ...
                <footer v-if="cart.items.length > 0" class="border-t border-neutral-200 px-5 py-4">
                    ...
                    <a :href="cartUrl" class="btn-brand w-full">Fechar pedido</a>
                    <button class="mt-2 w-full text-sm text-neutral-500 hover:text-neutral-700" @click="closeDrawer">Continuar no cardápio</button>
                </footer>
            </aside>
        </Transition>
    </Teleport>
