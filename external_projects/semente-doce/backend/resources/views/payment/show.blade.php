        ...
        <div class="mx-auto max-w-4xl">
            ...
            <div class="grid gap-6 lg:grid-cols-5">
                ...
                <div class="lg:col-span-3">
                    ...
                    <div id="pay-loading" class="card p-8 text-center text-sm text-neutral-400">Carregando formas de pagamento…</div>
                    <p id="pay-error" class="mt-3 hidden rounded-xl bg-red-50 px-4 py-3 text-sm text-red-700"></p>

                    {{-- Resultado PIX (QR code) --}}
                    <div id="pix-result" class="card hidden p-6 text-center">
                        <h2 class="text-lg font-extrabold text-neutral-900">Pague com PIX 💸</h2>
                        <p class="mt-1 text-sm text-neutral-500">Escaneie o QR code ou copie o código. Assim que o pagamento cair, seu pedido entra na cozinha. 🧁</p>
                        <img id="pix-qr" src="" alt="QR code PIX" class="mx-auto mt-4 h-56 w-56 rounded-2xl border border-neutral-100">
                        <div class="mt-4">
                ...
                <aside class="lg:col-span-2">
                    <div class="card p-6">
                        ...
                        <p class="mt-1 text-xs text-neutral-500">
                            {{ $order->fulfillment_label }}@if($order->isPickup()) na loja @elseif($order->delivery_neighborhood) · {{ $order->delivery_neighborhood }}@endif
                        </p>

                        <div class="mt-4 space-y-3">
                            @foreach($order->items as $item)
                                <div class="flex items-center gap-3">
                                    <img src="{{ $item->image_path }}" alt="" class="h-12 w-12 rounded-xl object-cover">
                                    <div class="min-w-0 flex-1">
                                        <p class="truncate text-sm font-medium text-neutral-800">{{ $item->title }}</p>
                                        @if($item->options_summary)
                                            <p class="truncate text-xs text-neutral-400">{{ $item->options_summary }}</p>
                                        @endif
                        ...
                        <dl class="mt-4 space-y-1.5 border-t border-neutral-100 pt-4 text-sm">
                            ...
                            <div class="flex justify-between"><dt class="text-neutral-500">Entrega</dt><dd>{{ (float) $order->delivery_fee === 0.0 ? 'Grátis' : money($order->delivery_fee) }}</dd></div>
                            <div class="flex justify-between border-t border-neutral-100 pt-2 text-base"><dt class="font-medium text-neutral-600">Total</dt><dd class="font-extrabold text-brand-700">{{ money($order->total) }}</dd></div>
                        </dl>
                    </div>
                ...
                onSubmit: ({ formData }) => {
                    ...
                    return fetch(@json(route('payment.process', $order)), {
                        ...
                        .then(({ ok, data }) => {
                            ...
                            try { localStorage.removeItem('semente.cart.v1'); } catch (e) {}
