    ...
    <div class="container-doce py-8">
            ...
            <a href="{{ route('orders.show', $order) }}" class="card mb-4 block overflow-hidden transition hover:border-brand-300">
                <div class="flex flex-wrap items-center justify-between gap-2 border-b border-neutral-100 bg-neutral-50 px-5 py-3 text-sm">
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        <span class="chip bg-neutral-100 text-neutral-600">{{ $order->fulfillment_label }}</span>
                    </div>
                ...
                <div class="flex items-center gap-4 p-5">
                    <div class="flex -space-x-3">
                        @foreach($order->items->take(4) as $item)
                            <img src="{{ $item->image_path }}" alt="" class="h-12 w-12 rounded-xl border-2 border-white object-cover">
                        @endforeach
                    </div>
                    <div class="flex-1">
                        <p class="text-sm text-neutral-600">{{ $order->items->count() }} {{ \Illuminate\Support\Str::plural('item', $order->items->count()) }}</p>
                    </div>
        ...
        @empty
            <div class="card flex flex-col items-center justify-center py-16 text-center">
                ...
                <a href="{{ route('home') }}" class="btn-brand mt-6">Ver o cardápio</a>
            </div>
        @endforelse
