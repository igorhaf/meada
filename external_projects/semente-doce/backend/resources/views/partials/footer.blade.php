...
<footer class="mt-16 bg-neutral-900 text-neutral-300">
    ...
    <div class="border-b border-white/10">
        <div class="container-doce flex flex-wrap items-center justify-evenly gap-x-6 gap-y-6 py-8">
            @foreach([
                ['👩‍🍳', 'Feito à mão', 'Receitas artesanais, todo dia'],
                ['🛵', 'Entrega no bairro', 'Quentinho na sua porta'],
                ['💳', 'Pague como quiser', 'Pix, cartão ou boleto'],
            ] as [$icon, $title, $desc])
    ...
    <div class="container-doce grid grid-cols-2 gap-8 py-12 md:grid-cols-4 lg:grid-cols-5">
        <div class="col-span-2">
            ...
            @if($settings->address)
                <p class="mt-4 text-sm text-neutral-400">📍 {{ $settings->address }}</p>
            @endif
            @if($settings->opening_hours)
                <p class="mt-1 whitespace-pre-line text-sm text-neutral-400">🕑 {{ $settings->opening_hours }}</p>
            @endif
        ...
        <div>
            <h3 class="mb-4 text-sm font-bold uppercase tracking-wide text-white">Cardápio</h3>
            <ul class="space-y-2 text-sm">
                <li><a href="{{ route('kits.index') }}" class="text-neutral-400 transition hover:text-brand-400">Kits & cestas</a></li>
        ...
        <div>
            <h3 class="mb-4 text-sm font-bold uppercase tracking-wide text-white">Encomendas</h3>
            <ul class="space-y-2 text-sm">
                <li><a href="{{ route('custom-orders.create') }}" class="text-neutral-400 hover:text-brand-400">Fazer uma encomenda</a></li>
                <li><a href="{{ route('pages.how') }}" class="text-neutral-400 hover:text-brand-400">Como encomendar</a></li>
            </ul>
        </div>
    ...
    <div class="border-t border-white/10">
        <div class="container-doce flex flex-col items-center justify-between gap-2 py-6 text-xs text-neutral-500 sm:flex-row">
            <p>© {{ date('Y') }} {{ $settings->site_name }} · Doces & salgados artesanais. Todos os direitos reservados.</p>
            <p>Feito com 🍫 pela <a href="https://meadadigital.com" target="_blank" rel="noopener" class="font-medium text-neutral-400 hover:text-brand-400">meada digital</a></p>
        </div>
    </div>
</footer>
