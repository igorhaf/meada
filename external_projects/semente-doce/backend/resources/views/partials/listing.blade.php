{{-- Listagem compartilhada da vitrine. Espera: $title, $products (paginator), $facets, $action. --}}
@php
    $sortOptions = [
        ...
        'newest' => 'Novidades',
        'best_selling' => 'Mais pedidos',
        'rating' => 'Melhor avaliados',
    ];
    $selected = fn ($key, $value) => in_array((string) $value, array_map('strval', (array) request($key, [])), true);
    $hasFilters = collect(['availability', 'unit', 'min', 'max'])->contains(fn ($k) => request()->filled($k));
@endphp
...
<div class="container-doce py-6">
    ...
    <div class="mb-6 flex flex-wrap items-end justify-between gap-3">
        <div>
            ...
            <p class="text-sm text-neutral-500">{{ $products->total() }} {{ \Illuminate\Support\Str::plural('item', $products->total()) }} no cardápio</p>
        </div>
        ...
        <aside>
            <details data-filters class="rounded-2xl border border-neutral-200 bg-white lg:sticky lg:top-24">
                ...
                <form method="GET" action="{{ $action }}" class="space-y-5 border-t border-neutral-100 p-4">
                    ...
                    {{-- Disponibilidade --}}
                    <div>
                        <h3 class="mb-2 text-sm font-bold text-neutral-800">Disponibilidade</h3>
                        <div class="space-y-1.5">
                            @foreach(['ready' => '⚡ Pronta-entrega', 'order' => '🎂 Sob encomenda'] as $value => $label)
                                <label class="flex cursor-pointer items-center gap-2 text-sm text-neutral-600">
                                    <input type="radio" name="availability" value="{{ $value }}" @checked(request('availability') === $value) onchange="this.form.submit()" class="border-neutral-300 text-brand-600 focus:ring-brand-500">
                                    {{ $label }}
                                    <span class="ml-auto text-xs text-neutral-400">{{ $facets['availability'][$value] ?? 0 }}</span>
                                </label>
                            @endforeach
                            <label class="flex cursor-pointer items-center gap-2 text-sm text-neutral-600">
                                <input type="radio" name="availability" value="" @checked(! request('availability')) onchange="this.form.submit()" class="border-neutral-300 text-brand-600 focus:ring-brand-500">
                                Todos
                            </label>
                        </div>
                    </div>
                    ...
                    {{-- Unidade de venda --}}
                    @if(count($facets['units']))
                        <div>
                            <h3 class="mb-2 text-sm font-bold text-neutral-800">Vendido por</h3>
                            <div class="flex flex-wrap gap-2">
                                @foreach($facets['units'] as $unit)
                                    <label class="cursor-pointer">
                                        <input type="checkbox" name="unit[]" value="{{ $unit }}" @checked($selected('unit', $unit)) onchange="this.form.submit()" class="peer sr-only">
                                        <span class="inline-block rounded-lg border border-neutral-300 px-3 py-1.5 text-xs font-medium text-neutral-600 peer-checked:border-brand-600 peer-checked:bg-brand-50 peer-checked:text-brand-700">{{ $facets['unitLabels'][$unit] ?? $unit }}</span>
                                    </label>
                                @endforeach
                            </div>
                        </div>
        ...
        <div>
            @if($products->isEmpty())
                <div class="flex flex-col items-center justify-center rounded-2xl border border-dashed border-neutral-300 bg-white py-20 text-center">
                    ...
                    <p class="mt-4 text-lg font-semibold text-neutral-700">Nada por aqui ainda</p>
                    <p class="mt-1 text-sm text-neutral-500">Tente ajustar os filtros ou buscar por outro docinho.</p>
...
<script>
    // Abre o painel de filtros por padrão no desktop; colapsado sob "Filtros" no mobile.
