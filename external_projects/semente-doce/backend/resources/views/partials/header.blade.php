...
<header class="sticky top-0 z-40">
    ...
    @if($settings->announcement)
        <div class="bg-brand-900 text-center text-xs font-medium text-brand-50 sm:text-sm">
            <div class="container-doce py-2">{{ $settings->announcement }}</div>
        </div>
    @endif

    {{-- Main bar --}}
    <div class="bg-brand-700 text-white shadow-sm">
        <div class="container-doce flex items-center gap-3 py-3 sm:gap-6">
            ...
            <div class="flex-1">
                ...
                >
                    ...
                    <form action="{{ route('search') }}" method="GET" class="flex overflow-hidden rounded-full bg-white">
                        <input name="q" value="{{ request('q') }}" placeholder="Busque por brigadeiro, coxinha, bolo…" class="w-full border-0 px-4 py-3 text-sm text-neutral-800 focus:outline-none">
                        <button class="bg-brand-600 px-4 text-white">Buscar</button>
                    </form>
                </div>
            </div>

            {{-- Destaque: encomendas (recurso estrela) --}}
            <a href="{{ route('custom-orders.create') }}" class="hidden shrink-0 items-center gap-2 rounded-full bg-caramel-500 px-4 py-2 text-sm font-semibold text-white transition hover:bg-caramel-600 md:inline-flex">
                🎂 Encomendar
            </a>
            ...
            @auth
                ...
                <details data-close-outside class="account-menu group relative hidden shrink-0 lg:block">
                    <summary class="flex cursor-pointer list-none items-center gap-2 rounded-full px-3 py-2 text-sm font-medium text-white/90 transition hover:bg-white/10 [&::-webkit-details-marker]:hidden">
                    ...
                    <div class="absolute right-0 top-full z-50 mt-1 w-56 overflow-hidden rounded-2xl border border-neutral-100 bg-white py-2 shadow-xl">
                        ...
                        <a href="{{ route('custom-orders.index') }}" class="menu-item">🎂 Minhas encomendas</a>
                        @if($u->isRoot())
                            <a href="{{ route('admin.dashboard') }}" class="menu-item font-semibold text-caramel-700">⚙️ Cozinha (painel)</a>
                        @endif
            ...
            @else
                <div class="hidden shrink-0 items-center gap-2 lg:flex">
                    <a href="{{ route('login') }}" class="rounded-full px-3 py-2 text-sm font-medium text-white/90 transition hover:bg-white/10">Entrar</a>
                    <a href="{{ route('register') }}" class="rounded-full bg-white/15 px-3 py-2 text-sm font-semibold text-white transition hover:bg-white/25">Cadastrar</a>
                </div>
            @endauth

            <div data-island="CartButton" class="shrink-0">
                <a href="{{ route('cart') }}" class="inline-flex items-center gap-2 rounded-full px-3 py-2 text-white/90 hover:bg-white/10">Sacola</a>
            </div>
        </div>
    </div>

    {{-- Category nav — chips com submenu no hover --}}
    <nav class="border-b border-neutral-200 bg-white">
        <div class="container-doce flex items-center gap-1 overflow-x-auto no-scrollbar xl:justify-center xl:overflow-visible">
            <a href="{{ route('kits.index') }}" class="flex items-center gap-1.5 whitespace-nowrap px-3 py-3 text-sm font-semibold text-brand-700 transition hover:text-brand-800">🎁 Kits</a>
            <a href="{{ route('custom-orders.create') }}" class="flex items-center gap-1.5 whitespace-nowrap px-3 py-3 text-sm font-semibold text-caramel-600 transition hover:text-caramel-700 md:hidden">🎂 Encomendar</a>
...
<script>
    // Fecha qualquer dropdown aberto (conta, categorias) ao clicar fora dele.
