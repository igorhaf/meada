{{-- Card de produto reutilizável. Espera $product (com images carregadas). --}}
<div class="card group flex h-full flex-col overflow-hidden">
    <a href="{{ $product->url }}" class="relative block aspect-square overflow-hidden bg-neutral-100">
        ...
        <div class="absolute left-2 top-2 flex flex-col gap-1">
            @if($product->discount_percent)
                <span class="chip bg-caramel-500 text-white shadow-sm">-{{ $product->discount_percent }}%</span>
            @endif
        </div>

        @if($product->is_made_to_order)
            <span class="absolute bottom-2 left-2 chip bg-white/95 text-caramel-600 shadow-sm">🎂 Sob encomenda</span>
        @else
            <span class="absolute bottom-2 left-2 chip bg-white/95 text-pistache-600 shadow-sm">⚡ Pronta-entrega</span>
        @endif
    </a>
    ...
    <div class="flex flex-1 flex-col p-3">
        <div class="mb-1 flex items-center gap-2 text-xs text-neutral-400">
            ...
            @if($product->serves)<span class="truncate">{{ $product->serves }}</span>@endif
        </div>
        ...
        @if($product->flavor)
            <p class="mt-0.5 truncate text-xs text-neutral-500">{{ $product->flavor }}</p>
        @endif

        @if($product->reviews_count > 0)
            <div class="mt-1 flex items-center gap-1 text-xs text-caramel-500">
        ...
        <div class="mt-auto pt-3">
            ...
            @endif
            <p class="text-lg font-extrabold text-neutral-900">{{ money($product->price) }} <span class="text-xs font-medium text-neutral-500">{{ $product->price_suffix }}</span></p>
