package com.meada.profiles.escola.visits;
...
/**
 * Dispara a notificação outbound ao responsável quando a visita muda de status (camada 8.19,
 * ESCAPADA 2). Best-effort: falha NUNCA reverte. Persiste em {@code messages} (outbound/human).
 * conversationId null → skip.
 */
@Component
public class EscolaVisitNotifier {

    private static final Logger log = LoggerFactory.getLogger(EscolaVisitNotifier.class);
    ...
    public EscolaVisitNotifier(ConversationRepository conversationRepository,
    ...
    public void notifyStatus(UUID companyId, UUID conversationId, String text) {
        ...
        try {
            ...
            if (phone.isEmpty() || phone.get().isBlank() || creds.isEmpty()) {
                log.warn("escola: visita sem canal resolúvel (phone/creds) p/ conversa {} — notificação não enviada",
                    conversationId);
                return;
            }
            ...
            log.warn("escola: falha ao notificar visita p/ conversa {} ({}) — visita segue mesmo assim",
