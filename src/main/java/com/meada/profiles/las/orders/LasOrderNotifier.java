...
/**
 * Dispara a notificação outbound à cliente quando o pedido las muda de status (camada 8.23). Clone
 * literal de {@link com.meada.profiles.lingerie.orders.LingerieOrderNotifier} (só o prefixo
 ...
 */
@Component
public class LasOrderNotifier {

    private static final Logger log = LoggerFactory.getLogger(LasOrderNotifier.class);
    ...
    public LasOrderNotifier(ConversationRepository conversationRepository,
    ...
    public void notifyStatus(UUID companyId, UUID conversationId, String text) {
        ...
        try {
            ...
            if (phone.isEmpty() || phone.get().isBlank() || creds.isEmpty()) {
                log.warn("las: pedido sem canal resolúvel (phone/creds) p/ conversa {} — notificação não enviada",
                    conversationId);
                return;
            }
            ...
            log.warn("las: falha ao notificar status p/ conversa {} ({}) — pedido segue mesmo assim",
