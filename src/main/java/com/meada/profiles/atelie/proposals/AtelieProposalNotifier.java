...
/**
 * Dispara a notificação outbound ao cliente quando a proposta de ateliê muda de status (camada
 * 8.14): orcada (com total), aprovada, fechada, recusada. Best-effort: falha NUNCA reverte a
 * transição. Persiste em {@code messages} (outbound/human). conversationId null → skip. Texto
 * defensivo, SEM promessa de resultado estético/prazo cravado. Espelho do EventProposalNotifier.
 */
@Component
public class AtelieProposalNotifier {

    private static final Logger log = LoggerFactory.getLogger(AtelieProposalNotifier.class);
    ...
    public AtelieProposalNotifier(ConversationRepository conversationRepository,
    ...
    public void notifyStatus(UUID companyId, UUID conversationId, String text) {
        ...
        try {
            ...
            if (phone.isEmpty() || phone.get().isBlank() || creds.isEmpty()) {
                log.warn("atelie: proposta sem canal resolúvel p/ conversa {} — notificação não enviada", conversationId);
                return;
            }
            ...
            log.warn("atelie: falha ao notificar status p/ conversa {} ({}) — proposta segue mesmo assim",
