...
/**
 * Dispara a notificação outbound ao paciente quando a consulta muda de status (camada 8.0):
 * confirmado (com tipo/profissional/data/hora) e cancelado. Best-effort: falha NUNCA reverte.
 * Persiste em {@code messages} (outbound/human). conversationId null → skip. Texto acolhedor, SEM
 * conteúdo nutricional.
 */
@Component
public class NutriAppointmentNotifier {

    private static final Logger log = LoggerFactory.getLogger(NutriAppointmentNotifier.class);
    ...
    public NutriAppointmentNotifier(ConversationRepository conversationRepository,
    ...
    public void notifyStatus(UUID companyId, UUID conversationId, String text) {
        ...
        try {
            ...
            if (phone.isEmpty() || phone.get().isBlank() || creds.isEmpty()) {
                log.warn("nutri: consulta sem canal resolúvel p/ conversa {} — notificação não enviada", conversationId);
                return;
            }
            ...
            log.warn("nutri: falha ao notificar status p/ conversa {} ({}) — consulta segue mesmo assim",
                conversationId, e.getMessage());
        }
    }

    /** Envia um texto outbound arbitrário (usado pela ENTREGA do plano — o body exato do plano). */
    public boolean sendText(UUID companyId, UUID conversationId, String text) {
        ...
        try {
            ...
            Optional<EvolutionCredentials> creds = instanceId.flatMap(whatsappInstanceRepository::findEvolutionCredentials);
            if (phone.isEmpty() || phone.get().isBlank() || creds.isEmpty()) {
                log.warn("nutri: entrega de plano sem canal resolúvel p/ conversa {} — não enviada", conversationId);
                return false;
            }
            String keyId = evolutionSender.sendText(creds.get().instanceName(), creds.get().token(), phone.get(), text);
            messageRepository.insertIfNew(companyId, conversationId, MessageDirection.OUTBOUND, MessageSender.HUMAN, text, keyId);
            ...
            log.warn("nutri: falha ao entregar plano p/ conversa {} ({})", conversationId, e.getMessage());
            return false;
        }
    }
}
