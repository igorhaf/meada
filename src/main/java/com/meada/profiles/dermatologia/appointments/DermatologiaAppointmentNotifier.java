...
/**
 * Dispara a notificação outbound ao paciente quando a consulta muda de status (camada 8.11):
 * confirmada (com tipo/profissional/data/hora) e cancelada. Best-effort: falha NUNCA reverte.
 * Persiste em {@code messages} (outbound/human). conversationId null → skip. Texto acolhedor, SEM
 * conteúdo clínico. {@link #sendText} é usado pela ENTREGA da nota de preparo (texto VERBATIM).
 */
@Component
public class DermatologiaAppointmentNotifier {

    private static final Logger log = LoggerFactory.getLogger(DermatologiaAppointmentNotifier.class);
    ...
    public DermatologiaAppointmentNotifier(ConversationRepository conversationRepository,
    ...
    public void notifyStatus(UUID companyId, UUID conversationId, String text) {
        ...
        try {
            ...
            if (phone.isEmpty() || phone.get().isBlank() || creds.isEmpty()) {
                log.warn("dermatologia: consulta sem canal resolúvel p/ conversa {} — notificação não enviada", conversationId);
                return;
            }
            ...
            log.warn("dermatologia: falha ao notificar status p/ conversa {} ({}) — consulta segue mesmo assim",
                conversationId, e.getMessage());
        }
    }

    /** Envia um texto outbound arbitrário (usado pela ENTREGA da nota de preparo — o texto exato). */
    public boolean sendText(UUID companyId, UUID conversationId, String text) {
        ...
        try {
            ...
            Optional<EvolutionCredentials> creds = instanceId.flatMap(whatsappInstanceRepository::findEvolutionCredentials);
            if (phone.isEmpty() || phone.get().isBlank() || creds.isEmpty()) {
                log.warn("dermatologia: entrega de preparo sem canal resolúvel p/ conversa {} — não enviada", conversationId);
                return false;
            }
            String keyId = evolutionSender.sendText(creds.get().instanceName(), creds.get().token(), phone.get(), text);
            messageRepository.insertIfNew(companyId, conversationId, MessageDirection.OUTBOUND, MessageSender.HUMAN, text, keyId);
            ...
            log.warn("dermatologia: falha ao entregar preparo p/ conversa {} ({})", conversationId, e.getMessage());
            return false;
        }
    }
}
