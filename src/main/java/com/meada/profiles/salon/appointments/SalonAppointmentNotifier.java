...
/**
 * Dispara a notificação outbound ao cliente quando o agendamento muda de status (camada 7.5,
 * decisão 3). Espelho dos notifiers anteriores. Best-effort: falha NUNCA reverte a transição. A
 * mensagem é persistida em {@code messages} (outbound/human — é o salão avisando). conversationId
 * null → skip silencioso. Texto defensivo (sem promessa estética).
 */
@Component
public class SalonAppointmentNotifier {

    private static final Logger log = LoggerFactory.getLogger(SalonAppointmentNotifier.class);
    ...
    public SalonAppointmentNotifier(ConversationRepository conversationRepository,
    ...
    public void notifyStatus(UUID companyId, UUID conversationId, String text) {
        ...
        try {
            ...
            if (phone.isEmpty() || phone.get().isBlank() || creds.isEmpty()) {
                log.warn("salon: agendamento sem canal resolúvel (phone/creds) p/ conversa {} — notificação não enviada",
                    conversationId);
                return;
            }
            ...
            log.warn("salon: falha ao notificar status p/ conversa {} ({}) — agendamento segue mesmo assim",
