...
/**
 * Dispara a notificação outbound ao cliente quando a OS muda de status (camada 7.9): orcada (com
 * total), aprovada, concluida, entregue. Best-effort: falha NUNCA reverte a transição. Persiste em
 * {@code messages} (outbound/human). conversationId null → skip. Texto defensivo, SEM diagnóstico
 * nem promessa de prazo.
 */
@Component
public class ServiceOrderNotifier {

    private static final Logger log = LoggerFactory.getLogger(ServiceOrderNotifier.class);
    ...
    public ServiceOrderNotifier(ConversationRepository conversationRepository,
    ...
    public void notifyStatus(UUID companyId, UUID conversationId, String text) {
        ...
        try {
            ...
            if (phone.isEmpty() || phone.get().isBlank() || creds.isEmpty()) {
                log.warn("oficina: OS sem canal resolúvel p/ conversa {} — notificação não enviada", conversationId);
                return;
            }
            ...
            log.warn("oficina: falha ao notificar status p/ conversa {} ({}) — OS segue mesmo assim",
