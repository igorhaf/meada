...
/**
 * Dispara a notificação outbound ao cliente quando a sessão muda de status (camada 8.16): confirmada
 * (com pacote/profissional/data/hora) e cancelada. Best-effort: falha NUNCA reverte. Persiste em
 * {@code messages} (outbound/human). conversationId null → skip. {@link #sendText} é usado pela
 * ENTREGA do material (delivery_link VERBATIM). Clone do DermatologiaAppointmentNotifier.
 */
@Component
public class FotografiaAppointmentNotifier {

    private static final Logger log = LoggerFactory.getLogger(FotografiaAppointmentNotifier.class);
    ...
    public FotografiaAppointmentNotifier(ConversationRepository conversationRepository,
    ...
    public void notifyStatus(UUID companyId, UUID conversationId, String text) {
        ...
        try {
            ...
            if (phone.isEmpty() || phone.get().isBlank() || creds.isEmpty()) {
                log.warn("fotografia: sessão sem canal resolúvel p/ conversa {} — notificação não enviada", conversationId);
                return;
            }
            ...
            log.warn("fotografia: falha ao notificar status p/ conversa {} ({}) — sessão segue mesmo assim",
                conversationId, e.getMessage());
        }
    }

    /** Envia um texto outbound arbitrário (usado pela ENTREGA do material — o delivery_link exato). */
    public boolean sendText(UUID companyId, UUID conversationId, String text) {
        ...
        try {
            ...
            Optional<EvolutionCredentials> creds = instanceId.flatMap(whatsappInstanceRepository::findEvolutionCredentials);
            if (phone.isEmpty() || phone.get().isBlank() || creds.isEmpty()) {
                log.warn("fotografia: entrega de material sem canal resolúvel p/ conversa {} — não enviada", conversationId);
                return false;
            }
            String keyId = evolutionSender.sendText(creds.get().instanceName(), creds.get().token(), phone.get(), text);
            messageRepository.insertIfNew(companyId, conversationId, MessageDirection.OUTBOUND, MessageSender.HUMAN, text, keyId);
            ...
            log.warn("fotografia: falha ao entregar material p/ conversa {} ({})", conversationId, e.getMessage());
            return false;
        }
    }
}
