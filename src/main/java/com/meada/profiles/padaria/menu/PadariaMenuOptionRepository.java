...
/**
 * Acesso a {@code padaria_menu_item_options} (camada 8.8 / perfil padaria, ESCAPADA 2). Clone de
 * {@link com.meada.profiles.floricultura.catalog.FloriculturaCatalogOptionRepository}
 * (catalog→menu: a coluna FK é {@code menu_item_id}). Opera via service_role; o escopo por company_id
 * (e por menu_item_id) no WHERE de cada query é a defesa.
 */
@Repository
public class PadariaMenuOptionRepository {

    private static final RowMapper<PadariaMenuOption> MAPPER = (rs, rn) -> new PadariaMenuOption(
    ...
    public PadariaMenuOptionRepository(JdbcTemplate jdbcTemplate) {
        this.jdbcTemplate = jdbcTemplate;
    }

    /** Todas as opções de um item (qualquer disponibilidade), ordenadas por sort_order. */
    public List<PadariaMenuOption> listByItem(UUID companyId, UUID menuItemId) {
        return jdbcTemplate.query(
            "select " + COLS + " from padaria_menu_item_options "
                + "where company_id = ? and menu_item_id = ? order by sort_order asc, created_at asc",
            MAPPER, companyId, menuItemId);
    }

    public Optional<PadariaMenuOption> findById(UUID companyId, UUID menuItemId, UUID id) {
        return jdbcTemplate.query(
                "select " + COLS + " from padaria_menu_item_options "
     ...
     */
    public List<PadariaMenuOption> findByIdsForItem(UUID companyId, UUID menuItemId,
    ...
    public PadariaMenuOption insert(UUID companyId, UUID menuItemId, String groupLabel,
                                    String optionLabel, int priceDeltaCents, int sortOrder) {
        UUID id = jdbcTemplate.queryForObject(
            "insert into padaria_menu_item_options (company_id, menu_item_id, group_label, "
     ...
     */
    public Optional<PadariaMenuOption> update(UUID companyId, UUID menuItemId, UUID id,
        ...
        if (!sets.isEmpty()) {
            ...
            int n = jdbcTemplate.update(
                "update padaria_menu_item_options set " + String.join(", ", sets)
                    + " where company_id = ? and menu_item_id = ? and id = ?",
                args.toArray());
            if (n == 0) {
    ...
    public Optional<PadariaMenuOption> toggle(UUID companyId, UUID menuItemId, UUID id, boolean available) {
        int n = jdbcTemplate.update(
            "update padaria_menu_item_options set available = ?, updated_at = now() "
                + "where company_id = ? and menu_item_id = ? and id = ?",
            available, companyId, menuItemId, id);
        return n == 0 ? Optional.empty() : findById(companyId, menuItemId, id);
    }

    /** Hard delete da opção. Snapshots em padaria_order_item_options são preservados (set null). */
    public boolean delete(UUID companyId, UUID menuItemId, UUID id) {
        return jdbcTemplate.update(
            "delete from padaria_menu_item_options where company_id = ? and menu_item_id = ? and id = ?",
