...
/**
 * Dispara a notificação outbound ao cliente quando o pedido pizzaria muda de status (camada 8.4).
 ...
 */
@Component
public class PizzariaOrderNotifier {

    private static final Logger log = LoggerFactory.getLogger(PizzariaOrderNotifier.class);
    ...
    public PizzariaOrderNotifier(ConversationRepository conversationRepository,
    ...
    public void notifyStatus(UUID companyId, UUID conversationId, String text) {
        ...
        try {
            ...
            if (phone.isEmpty() || phone.get().isBlank() || creds.isEmpty()) {
                log.warn("pizzaria: pedido sem canal resolúvel (phone/creds) p/ conversa {} — notificação não enviada",
                    conversationId);
                return;
            }
            ...
            log.warn("pizzaria: falha ao notificar status p/ conversa {} ({}) — pedido segue mesmo assim",
