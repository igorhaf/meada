...
/**
 * Dispara a notificação outbound ao cliente quando o pedido adega muda de status (camada 8.4).
 ...
 */
@Component
public class AdegaOrderNotifier {

    private static final Logger log = LoggerFactory.getLogger(AdegaOrderNotifier.class);
    ...
    public AdegaOrderNotifier(ConversationRepository conversationRepository,
    ...
    public void notifyStatus(UUID companyId, UUID conversationId, String text) {
        ...
        try {
            ...
            if (phone.isEmpty() || phone.get().isBlank() || creds.isEmpty()) {
                log.warn("adega: pedido sem canal resolúvel (phone/creds) p/ conversa {} — notificação não enviada",
                    conversationId);
                return;
            }
            ...
            log.warn("adega: falha ao notificar status p/ conversa {} ({}) — pedido segue mesmo assim",
