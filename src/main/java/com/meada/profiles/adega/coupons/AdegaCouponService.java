...
/**
 * Regras dos cupons adega (camada 8.9, backlog #1 — clone do chassi sushi). Valida code/kind/value
 ...
 * {@code AdegaOrderRepository.createOrder}, não aqui — aqui é só o CRUD do catálogo de cupons.
 */
@Service
public class AdegaCouponService {

    private final AdegaCouponRepository repository;
    private final AuditLogger auditLogger;

    public AdegaCouponService(AdegaCouponRepository repository, AuditLogger auditLogger) {
    ...
    @Transactional
    public AdegaCoupon create(UUID companyId, UUID userId, String code, String kind, int value,
        ...
        auditLogger.log(companyId, userId, "adega_coupon_created", "adega_coupon",
            created.id(), Map.of("code", created.code()));
        return created;
    }

    @Transactional
    public AdegaCoupon update(UUID companyId, UUID userId, UUID id, String code, String kind,
        ...
        if (kind != null || value != null) {
            AdegaCoupon current = repository.findById(companyId, id)
        ...
        auditLogger.log(companyId, userId, "adega_coupon_updated", "adega_coupon", id, Map.of());
        return updated;
    }

    @Transactional
    public AdegaCoupon toggle(UUID companyId, UUID userId, UUID id, boolean active) {
        AdegaCoupon c = repository.toggle(companyId, id, active)
            .orElseThrow(CouponNotFoundException::new);
        auditLogger.log(companyId, userId, "adega_coupon_updated", "adega_coupon", id,
    ...
    public void delete(UUID companyId, UUID userId, UUID id) {
        ...
        auditLogger.log(companyId, userId, "adega_coupon_deleted", "adega_coupon", id, Map.of());
    }

    public List<AdegaCoupon> list(UUID companyId) {
        return repository.listByCompany(companyId);
    }

    public Optional<AdegaCoupon> get(UUID companyId, UUID id) {
