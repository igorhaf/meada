...
import com.meada.profiles.adega.coupons.AdegaCouponService.CouponNotFoundException;
...
/**
 * Cupons de desconto do tenant adega (camada 8.9, backlog #1 — clone do chassi sushi). TENANT +
 * perfil 'adega' only. CRUD do catálogo de cupons; a aplicação acontece na criação do pedido. Sob
 * {@code /api/adega/**}.
 */
@RestController
public class AdegaCouponController {

    private final AdegaCouponService service;
    private final AdegaProfileGuard profileGuard;

    public AdegaCouponController(AdegaCouponService service, AdegaProfileGuard profileGuard) {
    ...
    @GetMapping("/api/adega/coupons/{id}")
    ...
    @PatchMapping("/api/adega/coupons/{id}")
    ...
    @PatchMapping("/api/adega/coupons/{id}/toggle")
    ...
    @DeleteMapping("/api/adega/coupons/{id}")
