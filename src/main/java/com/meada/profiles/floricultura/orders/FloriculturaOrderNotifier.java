...
/**
 * Dispara a notificação outbound ao cliente quando o pedido floricultura muda de status (camada 8.4).
 * Clone literal de {@link com.meada.profiles.sushi.orders.SushiOrderNotifier} (só o prefixo
 ...
 */
@Component
public class FloriculturaOrderNotifier {

    private static final Logger log = LoggerFactory.getLogger(FloriculturaOrderNotifier.class);
    ...
    public FloriculturaOrderNotifier(ConversationRepository conversationRepository,
    ...
    public void notifyStatus(UUID companyId, UUID conversationId, String text) {
        ...
        try {
            ...
            if (phone.isEmpty() || phone.get().isBlank() || creds.isEmpty()) {
                log.warn("floricultura: pedido sem canal resolúvel (phone/creds) p/ conversa {} — notificação não enviada",
                    conversationId);
                return;
            }
            ...
            log.warn("floricultura: falha ao notificar status p/ conversa {} ({}) — pedido segue mesmo assim",
