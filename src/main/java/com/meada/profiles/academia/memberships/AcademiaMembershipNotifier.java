...
/**
 * Dispara a notificação outbound ao aluno quando a matrícula muda de status (camada 7.7, decisão 3).
 * Espelho dos notifiers anteriores. Best-effort: falha NUNCA reverte. Persiste em {@code messages}
 * (outbound/human). conversationId null → skip. Texto defensivo, SEM promessa de resultado.
 */
@Component
public class AcademiaMembershipNotifier {

    private static final Logger log = LoggerFactory.getLogger(AcademiaMembershipNotifier.class);
    ...
    public AcademiaMembershipNotifier(ConversationRepository conversationRepository,
    ...
    public void notifyStatus(UUID companyId, UUID conversationId, String text) {
        ...
        try {
            ...
            if (phone.isEmpty() || phone.get().isBlank() || creds.isEmpty()) {
                log.warn("academia: matrícula sem canal resolúvel (phone/creds) p/ conversa {} — notificação não enviada",
                    conversationId);
                return;
            }
            ...
            log.warn("academia: falha ao notificar status p/ conversa {} ({}) — matrícula segue mesmo assim",
