...
/**
 * Dispara a notificação outbound aos noivos quando a proposta muda de status (camada 8.7): orcada
 * (com total), aprovada, fechada, recusada. Best-effort: falha NUNCA reverte a transição. Persiste em
 * {@code messages} (outbound/human). conversationId null → skip. Texto defensivo, SEM promessa de
 * "casamento perfeito". Espelho do EventProposalNotifier.
 */
@Component
public class WeddingProposalNotifier {

    private static final Logger log = LoggerFactory.getLogger(WeddingProposalNotifier.class);
    ...
    public WeddingProposalNotifier(ConversationRepository conversationRepository,
    ...
    public void notifyStatus(UUID companyId, UUID conversationId, String text) {
        ...
        try {
            ...
            if (phone.isEmpty() || phone.get().isBlank() || creds.isEmpty()) {
                log.warn("casamento: proposta sem canal resolúvel p/ conversa {} — notificação não enviada", conversationId);
                return;
            }
            ...
            log.warn("casamento: falha ao notificar status p/ conversa {} ({}) — proposta segue mesmo assim",
