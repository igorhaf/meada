...
/**
 * Regras dos cupons casamento (onda 1 do casamento, backlog #10 — clone do motor atelie). Valida code/kind/value
 ...
 * acontece no {@code WeddingProposalService.applyCoupon}, não aqui — aqui é só o CRUD do catálogo.
 */
@Service
public class WeddingCouponService {
    ...
    @Transactional
    public WeddingCoupon create(UUID companyId, UUID userId, String code, String kind, int value,
        ...
        auditLogger.log(companyId, userId, "wedding_coupon_created", "wedding_coupon",
            created.id(), Map.of("code", created.code()));
        return created;
    }

    @Transactional
    public WeddingCoupon update(UUID companyId, UUID userId, UUID id, String code, String kind,
        ...
        if (kind != null || value != null) {
            WeddingCoupon current = repository.findById(companyId, id)
        ...
        auditLogger.log(companyId, userId, "wedding_coupon_updated", "wedding_coupon", id, Map.of());
        return updated;
    }

    @Transactional
    public WeddingCoupon toggle(UUID companyId, UUID userId, UUID id, boolean active) {
        WeddingCoupon c = repository.toggle(companyId, id, active)
            .orElseThrow(CouponNotFoundException::new);
        auditLogger.log(companyId, userId, "wedding_coupon_updated", "wedding_coupon", id,
    ...
    public void delete(UUID companyId, UUID userId, UUID id) {
        ...
        auditLogger.log(companyId, userId, "wedding_coupon_deleted", "wedding_coupon", id, Map.of());
    }

    public List<WeddingCoupon> list(UUID companyId) {
        return repository.listByCompany(companyId);
    }

    public Optional<WeddingCoupon> get(UUID companyId, UUID id) {
