...
/**
 * Dispara a notificação outbound à cliente quando o pedido lingerie muda de status (camada 8.21).
 * Clone literal de {@link com.meada.whatsapp.profiles.adega.orders.AdegaOrderNotifier} (só o prefixo
 ...
 */
@Component
public class LingerieOrderNotifier {

    private static final Logger log = LoggerFactory.getLogger(LingerieOrderNotifier.class);
    ...
    public LingerieOrderNotifier(ConversationRepository conversationRepository,
    ...
    public void notifyStatus(UUID companyId, UUID conversationId, String text) {
        ...
        try {
            ...
            if (phone.isEmpty() || phone.get().isBlank() || creds.isEmpty()) {
                log.warn("lingerie: pedido sem canal resolúvel (phone/creds) p/ conversa {} — notificação não enviada",
                    conversationId);
                return;
            }
            ...
            log.warn("lingerie: falha ao notificar status p/ conversa {} ({}) — pedido segue mesmo assim",
