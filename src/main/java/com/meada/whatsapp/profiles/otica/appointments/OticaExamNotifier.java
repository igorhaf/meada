...
/**
 * Dispara a notificação outbound ao cliente quando o exame muda de status (camada 8.12, FLUXO A).
 * Clone do {@code DentalAppointmentNotifier}: resolve telefone + credenciais via a conversa e envia
 * pela Evolution. EVOLUTION_DRY_RUN (dev) é honrado pelo EvolutionSender.
 *
 * <p>Best-effort: falha de envio NUNCA reverte a transição (já persistida) — loga warn e segue. A
 * mensagem é persistida em {@code messages} (outbound/human — é a ótica avisando, não a IA).
 *
 * <p>{@code conversationId} null (exame manual sem WhatsApp) → pula em silêncio. Textos DEFENSIVOS
 * (sem promessa clínica) — ver {@link OticaExamStatus#notificationText}.
 */
@Component
public class OticaExamNotifier {

    private static final Logger log = LoggerFactory.getLogger(OticaExamNotifier.class);
    ...
    public OticaExamNotifier(ConversationRepository conversationRepository,
    ...
    public void notifyStatus(UUID companyId, UUID conversationId, String text) {
        if (text == null) {
            return;   // status que não notifica (agendado/realizado/falta).
        }
        if (conversationId == null) {
            return;   // exame manual sem WhatsApp — nada a notificar (silencioso).
        }
        try {
            ...
            if (phone.isEmpty() || phone.get().isBlank() || creds.isEmpty()) {
                log.warn("otica: exame sem canal resolúvel (phone/creds) p/ conversa {} — notificação não enviada",
                    conversationId);
                return;
            }
            ...
            log.warn("otica: falha ao notificar status do exame p/ conversa {} ({}) — exame segue mesmo assim",
