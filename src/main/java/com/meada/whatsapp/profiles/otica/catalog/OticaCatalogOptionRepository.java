...
/**
 * Acesso a {@code otica_catalog_item_options} (camada 8.12, FLUXO B). Opera via service_role; o
 * escopo por company_id (e por catalog_item_id) no WHERE é a defesa. Clone de
 * {@link com.meada.whatsapp.profiles.floricultura.catalog.FloriculturaCatalogOptionRepository}.
 */
@Repository
public class OticaCatalogOptionRepository {

    private static final RowMapper<OticaCatalogOption> MAPPER = (rs, rn) -> new OticaCatalogOption(
    ...
    public OticaCatalogOptionRepository(JdbcTemplate jdbcTemplate) {
        this.jdbcTemplate = jdbcTemplate;
    }

    /** Todas as opções de um item (qualquer disponibilidade), ordenadas por sort_order. */
    public List<OticaCatalogOption> listByItem(UUID companyId, UUID catalogItemId) {
        return jdbcTemplate.query(
            "select " + COLS + " from otica_catalog_item_options "
                + "where company_id = ? and catalog_item_id = ? order by sort_order asc, created_at asc",
            MAPPER, companyId, catalogItemId);
    }

    public Optional<OticaCatalogOption> findById(UUID companyId, UUID catalogItemId, UUID id) {
        return jdbcTemplate.query(
                "select " + COLS + " from otica_catalog_item_options "
     ...
     * difere do de optionIds, alguma opção é inválida/indisponível/de outro item.
     */
    public List<OticaCatalogOption> findByIdsForItem(UUID companyId, UUID catalogItemId,
    ...
    public OticaCatalogOption insert(UUID companyId, UUID catalogItemId, String groupLabel,
                                     String optionLabel, int priceDeltaCents, int sortOrder) {
        UUID id = jdbcTemplate.queryForObject(
            "insert into otica_catalog_item_options (company_id, catalog_item_id, group_label, "
    ...
    /** Atualiza campos não-null (PATCH parcial). Retorna empty se não existir/pertencer ao item+tenant. */
    public Optional<OticaCatalogOption> update(UUID companyId, UUID catalogItemId, UUID id,
        ...
        if (!sets.isEmpty()) {
            ...
            int n = jdbcTemplate.update(
                "update otica_catalog_item_options set " + String.join(", ", sets)
                    + " where company_id = ? and catalog_item_id = ? and id = ?",
                args.toArray());
            if (n == 0) {
    ...
    public Optional<OticaCatalogOption> toggle(UUID companyId, UUID catalogItemId, UUID id, boolean available) {
        int n = jdbcTemplate.update(
            "update otica_catalog_item_options set available = ?, updated_at = now() "
                + "where company_id = ? and catalog_item_id = ? and id = ?",
            available, companyId, catalogItemId, id);
        return n == 0 ? Optional.empty() : findById(companyId, catalogItemId, id);
    }

    /** Hard delete da opção. Snapshots em otica_order_item_options são preservados (set null). */
    public boolean delete(UUID companyId, UUID catalogItemId, UUID id) {
        return jdbcTemplate.update(
            "delete from otica_catalog_item_options where company_id = ? and catalog_item_id = ? and id = ?",
