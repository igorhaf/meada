...
/**
 * Dispara a notificação outbound ao cliente quando o pedido papelaria muda de status (camada 8.15 /
 * perfil papelaria). Clone literal de
 * {@link com.meada.whatsapp.profiles.padaria.orders.PadariaOrderNotifier} (camada 8.8; só o prefixo de
 ...
 * a papelaria avisando, não a IA), best-effort.
 */
@Component
public class PapelariaOrderNotifier {

    private static final Logger log = LoggerFactory.getLogger(PapelariaOrderNotifier.class);
    ...
    public PapelariaOrderNotifier(ConversationRepository conversationRepository,
    ...
    public void notifyStatus(UUID companyId, UUID conversationId, String text) {
        ...
        try {
            ...
            if (phone.isEmpty() || phone.get().isBlank() || creds.isEmpty()) {
                log.warn("papelaria: pedido sem canal resolúvel (phone/creds) p/ conversa {} — notificação não enviada",
                    conversationId);
                return;
            }
            String keyId = evolutionSender.sendText(
                creds.get().instanceName(), creds.get().token(), phone.get(), text);
            // Persiste a notificação como mensagem outbound (sender=human: é a papelaria, não a IA).
            messageRepository.insertIfNew(companyId, conversationId,
                MessageDirection.OUTBOUND, MessageSender.HUMAN, text, keyId);
        } catch (RuntimeException e) {
            log.warn("papelaria: falha ao notificar status p/ conversa {} ({}) — pedido segue mesmo assim",
