package com.meada.whatsapp.profiles.papelaria.catalog;
...
/**
 * Acesso a {@code papelaria_catalog_item_options} (camada 8.15 / perfil papelaria). Clone de
 * {@code com.meada.whatsapp.profiles.padaria.menu.PadariaMenuOptionRepository} (camada 8.8) —
 * menu→catalog: a coluna FK é {@code catalog_item_id}. Opera via service_role; o escopo por company_id
 * (e por catalog_item_id) no WHERE de cada query é a defesa.
 */
@Repository
public class PapelariaCatalogOptionRepository {

    private static final RowMapper<PapelariaCatalogOption> MAPPER = (rs, rn) -> new PapelariaCatalogOption(
    ...
    public PapelariaCatalogOptionRepository(JdbcTemplate jdbcTemplate) {
        this.jdbcTemplate = jdbcTemplate;
    }

    /** Todas as opções de um item (qualquer disponibilidade), ordenadas por sort_order. */
    public List<PapelariaCatalogOption> listByItem(UUID companyId, UUID catalogItemId) {
        return jdbcTemplate.query(
            "select " + COLS + " from papelaria_catalog_item_options "
                + "where company_id = ? and catalog_item_id = ? order by sort_order asc, created_at asc",
            MAPPER, companyId, catalogItemId);
    }

    public Optional<PapelariaCatalogOption> findById(UUID companyId, UUID catalogItemId, UUID id) {
        return jdbcTemplate.query(
                "select " + COLS + " from papelaria_catalog_item_options "
     ...
     * Entrada vazia (sem opções pedidas) → lista vazia.
     */
    public List<PapelariaCatalogOption> findByIdsForItem(UUID companyId, UUID catalogItemId,
    ...
    public PapelariaCatalogOption insert(UUID companyId, UUID catalogItemId, String groupLabel,
                                         String optionLabel, int priceDeltaCents, int sortOrder) {
        UUID id = jdbcTemplate.queryForObject(
            "insert into papelaria_catalog_item_options (company_id, catalog_item_id, group_label, "
     ...
     */
    public Optional<PapelariaCatalogOption> update(UUID companyId, UUID catalogItemId, UUID id,
        ...
        if (!sets.isEmpty()) {
            ...
            int n = jdbcTemplate.update(
                "update papelaria_catalog_item_options set " + String.join(", ", sets)
                    + " where company_id = ? and catalog_item_id = ? and id = ?",
                args.toArray());
            if (n == 0) {
    ...
    public Optional<PapelariaCatalogOption> toggle(UUID companyId, UUID catalogItemId, UUID id, boolean available) {
        int n = jdbcTemplate.update(
            "update papelaria_catalog_item_options set available = ?, updated_at = now() "
                + "where company_id = ? and catalog_item_id = ? and id = ?",
            available, companyId, catalogItemId, id);
        return n == 0 ? Optional.empty() : findById(companyId, catalogItemId, id);
    }

    /** Hard delete da opção. Snapshots em papelaria_order_item_options são preservados (set null). */
    public boolean delete(UUID companyId, UUID catalogItemId, UUID id) {
        return jdbcTemplate.update(
            "delete from papelaria_catalog_item_options where company_id = ? and catalog_item_id = ? and id = ?",
