...
/**
 * Status de um pedido floricultura (camada 8.4) — clone de
 ...
 * {@code frontend/profiles/floricultura/floricultura-order-status.ts} (FloriculturaOrderStatusParityTest).
 */
public enum FloriculturaOrderStatus {
    ...
    FloriculturaOrderStatus(String id) {
        this.id = id;
    }
    ...
    public static Optional<FloriculturaOrderStatus> fromId(String id) {
        if (id == null) {
    ...
    public Set<FloriculturaOrderStatus> allowedNext() {
        return switch (this) {
    ...
    public boolean canTransitionTo(FloriculturaOrderStatus next) {
    ...
    public String notificationText() {
        return switch (this) {
            case EM_PREPARO -> "Seu pedido foi confirmado! Já estamos preparando seu arranjo com carinho. 🌷";
            case SAIU_ENTREGA -> "Seu arranjo saiu para entrega. Em breve chega ao destino!";
            case ENTREGUE -> "Pedido entregue! Esperamos que as flores tragam um sorriso. 💐";
            case RECUSADO -> "Infelizmente não conseguimos atender seu pedido para essa data. Pedimos desculpa pelo transtorno.";
