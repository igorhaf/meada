...
/**
 * Dispara a notificação outbound ao paciente quando a consulta muda de status (camada 7.4,
 * decisão 2). Espelho dos notifiers anteriores: resolve telefone + credenciais via a conversa e
 * envia pela Evolution. EVOLUTION_DRY_RUN (dev) é honrado pelo EvolutionSender.
 *
 * <p>Best-effort: falha de envio NUNCA reverte a transição (já persistida) — loga warn e segue. A
 * mensagem é persistida em {@code messages} (outbound/human — é a clínica avisando, não a IA).
 *
 * <p>{@code conversationId} null (consulta manual sem WhatsApp) → pula em silêncio.
 *
 * <p>Os textos são DEFENSIVOS (sem promessa clínica) — ver {@link AppointmentStatus#notificationText}.
 */
@Component
public class DentalAppointmentNotifier {

    private static final Logger log = LoggerFactory.getLogger(DentalAppointmentNotifier.class);
    ...
    public DentalAppointmentNotifier(ConversationRepository conversationRepository,
    ...
    /** Envia o texto do status ao paciente da conversa. Best-effort (nunca lança). */
    public void notifyStatus(UUID companyId, UUID conversationId, String text) {
        if (text == null) {
            return;   // status que não notifica (agendada/realizada/falta).
        }
        if (conversationId == null) {
            return;   // consulta manual sem WhatsApp — nada a notificar (silencioso).
        }
        try {
            ...
            if (phone.isEmpty() || phone.get().isBlank() || creds.isEmpty()) {
                log.warn("dental: consulta sem canal resolúvel (phone/creds) p/ conversa {} — notificação não enviada",
                    conversationId);
                return;
            }
            ...
            log.warn("dental: falha ao notificar status p/ conversa {} ({}) — consulta segue mesmo assim",
