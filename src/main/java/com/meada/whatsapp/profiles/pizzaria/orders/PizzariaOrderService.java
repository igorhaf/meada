...
import com.meada.whatsapp.profiles.pizzaria.PizzariaConfig;
import com.meada.whatsapp.profiles.pizzaria.PizzariaConfigRepository;
...
/**
 * Regras dos pedidos pizzaria (camada 8.4). Clone de
 * {@link com.meada.whatsapp.profiles.sushi.orders.SushiOrderService} + a ESCAPADA 1 (rejection_reason
 * no gate de recusa).
 *
 * <p>{@link #create} é chamado pelo {@code PedidoPizzaConfirmHandler} (vindo da IA). Lê a taxa de
 * entrega do {@link PizzariaConfig} do tenant e delega ao repositório — que recalcula os totais a
 ...
 * notificação outbound do novo status via {@link PizzariaOrderNotifier}. O aceite/recusa é AÇÃO
 * HUMANA (a IA não transiciona).
 */
@Service
public class PizzariaOrderService {

    private static final Logger log = LoggerFactory.getLogger(PizzariaOrderService.class);

    private final PizzariaOrderRepository orderRepository;
    private final PizzariaConfigRepository configRepository;
    private final PizzariaOrderNotifier notifier;

    public PizzariaOrderService(PizzariaOrderRepository orderRepository,
                              PizzariaConfigRepository configRepository,
                              PizzariaOrderNotifier notifier) {
    ...
    public Optional<PizzariaOrder> get(UUID companyId, UUID id) {
     ...
     */
    @Transactional
    public PizzariaOrder updateStatus(UUID companyId, UUID id, String newStatusId, String rejectionReason) {
        PizzaOrderStatus newStatus = PizzaOrderStatus.fromId(newStatusId)
            .orElseThrow(InvalidStatusException::new);

        PizzariaOrder current = orderRepository.findById(companyId, id)
            .orElseThrow(OrderNotFoundException::new);
        PizzaOrderStatus from = PizzaOrderStatus.fromId(current.status())
            .orElseThrow(InvalidStatusException::new);
        ...
        String reasonToPersist = newStatus == PizzaOrderStatus.RECUSADO ? rejectionReason : null;
        ...
        if (newStatus == PizzaOrderStatus.RECUSADO && rejectionReason != null && !rejectionReason.isBlank()) {
