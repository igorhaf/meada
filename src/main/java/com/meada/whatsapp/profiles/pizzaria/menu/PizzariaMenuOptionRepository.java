...
/**
 * Acesso a {@code pizzaria_menu_item_options} (camada 8.4, ESCAPADA 2). Opera via service_role; o
 ...
 */
@Repository
public class PizzariaMenuOptionRepository {

    private static final RowMapper<PizzariaMenuOption> MAPPER = (rs, rn) -> new PizzariaMenuOption(
    ...
    public PizzariaMenuOptionRepository(JdbcTemplate jdbcTemplate) {
        this.jdbcTemplate = jdbcTemplate;
    }

    /** Todas as opções de um item (qualquer disponibilidade), ordenadas por sort_order. */
    public List<PizzariaMenuOption> listByItem(UUID companyId, UUID menuItemId) {
        return jdbcTemplate.query(
            "select " + COLS + " from pizzaria_menu_item_options "
                + "where company_id = ? and menu_item_id = ? order by sort_order asc, created_at asc",
            MAPPER, companyId, menuItemId);
    }

    public Optional<PizzariaMenuOption> findById(UUID companyId, UUID menuItemId, UUID id) {
        return jdbcTemplate.query(
                "select " + COLS + " from pizzaria_menu_item_options "
     ...
     */
    public List<PizzariaMenuOption> findByIdsForItem(UUID companyId, UUID menuItemId,
    ...
    public PizzariaMenuOption insert(UUID companyId, UUID menuItemId, String groupLabel,
                                   String optionLabel, int priceDeltaCents, int sortOrder) {
        UUID id = jdbcTemplate.queryForObject(
            "insert into pizzaria_menu_item_options (company_id, menu_item_id, group_label, "
     ...
     */
    public Optional<PizzariaMenuOption> update(UUID companyId, UUID menuItemId, UUID id,
        ...
        if (!sets.isEmpty()) {
            ...
            int n = jdbcTemplate.update(
                "update pizzaria_menu_item_options set " + String.join(", ", sets)
                    + " where company_id = ? and menu_item_id = ? and id = ?",
                args.toArray());
            if (n == 0) {
    ...
    public Optional<PizzariaMenuOption> toggle(UUID companyId, UUID menuItemId, UUID id, boolean available) {
        int n = jdbcTemplate.update(
            "update pizzaria_menu_item_options set available = ?, updated_at = now() "
                + "where company_id = ? and menu_item_id = ? and id = ?",
            available, companyId, menuItemId, id);
        return n == 0 ? Optional.empty() : findById(companyId, menuItemId, id);
    }

    /** Hard delete da opção. Snapshots em pizzaria_order_item_options são preservados (set null). */
    public boolean delete(UUID companyId, UUID menuItemId, UUID id) {
        return jdbcTemplate.update(
            "delete from pizzaria_menu_item_options where company_id = ? and menu_item_id = ? and id = ?",
