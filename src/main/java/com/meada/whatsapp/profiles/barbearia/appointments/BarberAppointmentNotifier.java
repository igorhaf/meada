...
/**
 * Dispara a notificação outbound ao cliente quando o agendamento OU o ticket da fila muda de estado
 * (camada 8.1). Espelho de SalonAppointmentNotifier — reusado pela agenda E pela fila (a chamada
 * "chegou sua vez"). Best-effort: falha NUNCA reverte a transição. A mensagem é persistida em
 * {@code messages} (outbound/human). conversationId null → skip silencioso. Texto defensivo.
 */
@Component
public class BarberAppointmentNotifier {

    private static final Logger log = LoggerFactory.getLogger(BarberAppointmentNotifier.class);
    ...
    public BarberAppointmentNotifier(ConversationRepository conversationRepository,
    ...
    public void notifyStatus(UUID companyId, UUID conversationId, String text) {
        ...
        try {
            ...
            if (phone.isEmpty() || phone.get().isBlank() || creds.isEmpty()) {
                log.warn("barbearia: sem canal resolúvel (phone/creds) p/ conversa {} — notificação não enviada",
                    conversationId);
                return;
            }
            ...
            log.warn("barbearia: falha ao notificar status p/ conversa {} ({}) — segue mesmo assim",
