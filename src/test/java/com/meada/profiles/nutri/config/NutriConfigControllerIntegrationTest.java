...
/**
 * Testa os endpoints de config do nutri (camada 8.0): GET default 08:00–18:00, PUT atualiza,
 * PUT com opens >= closes → 400 invalid_hours, profile guard 403.
 */
class NutriConfigControllerIntegrationTest extends AbstractAdminIntegrationTest {
    ...
    void getDefault() throws Exception {
        ...
        mockMvc.perform(get("/api/nutri/config").header("Authorization", "Bearer " + t))
    ...
    void putUpdates() throws Exception {
        ...
        mockMvc.perform(put("/api/nutri/config").header("Authorization", "Bearer " + t)
    ...
    @Test
    @DisplayName("tenant NÃO-nutri (pet) → /api/nutri/config → 403 forbidden_wrong_profile")
