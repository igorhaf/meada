...
/**
 * Testa os endpoints de pedidos do perfil floricultura (camada 8.4): list+filter, detail, e o GATE DE
 ...
 */
class FloriculturaOrderControllerIntegrationTest extends AbstractAdminIntegrationTest {
    ...
    /** Semeia um tenant 'floricultura' + um pedido 'aguardando' (default da migration). */
    private void seedFloriculturaOrder() {
        UUID sub = UUID.randomUUID();
        companyId = seedTenantAdmin("floricultura@test.dev", sub);
        jdbcTemplate.update("update companies set profile_id = 'floricultura' where id = ?", companyId);
        token = mintValidToken("floricultura@test.dev", sub);
        ...
        UUID catalogItem = jdbcTemplate.queryForObject(
            "insert into floricultura_catalog_items (company_id, name, price_cents, category) "
                + "values (?, 'X-Burger', 2500, 'buques') returning id", UUID.class, companyId);
        // pedido nasce 'aguardando' (default da coluna status) — não setar explicitamente.
        orderId = jdbcTemplate.queryForObject(
            "insert into floricultura_orders (company_id, conversation_id, contact_id, subtotal_cents, total_cents, delivery_address, delivery_date, delivery_period, recipient_name) "
                + "values (?, ?, ?, 5000, 5700, 'Rua Y 2', current_date + 1, 'manha', 'Maria') returning id", UUID.class, companyId, conv, contact);
        jdbcTemplate.update("insert into floricultura_order_items (order_id, catalog_item_id, qtd, unit_price_cents, item_name_snapshot) "
            + "values (?, ?, 2, 2500, 'X-Burger')", orderId, catalogItem);
    }

    @Test
    @DisplayName("GET lista (filtro status=aguardando) + detalhe com items e contato")
    void listAndDetail() throws Exception {
        seedFloriculturaOrder();
        mockMvc.perform(get("/api/floricultura/orders?status=aguardando").header("Authorization", "Bearer " + token))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.total").value(1))
            .andExpect(jsonPath("$.items[0].status").value("aguardando"));

        mockMvc.perform(get("/api/floricultura/orders/" + orderId).header("Authorization", "Bearer " + token))
            .andExpect(status().isOk())
    ...
    void patchStatusAccept() throws Exception {
        seedFloriculturaOrder();
        mockMvc.perform(patch("/api/floricultura/orders/" + orderId + "/status").header("Authorization", "Bearer " + token)
    ...
    @Test
    @DisplayName("tenant NÃO-floricultura (legal) batendo no /api/floricultura/orders → 403 forbidden_wrong_profile")
    void wrongProfile() throws Exception {
        ...
        mockMvc.perform(get("/api/floricultura/orders").header("Authorization", "Bearer " + t))
