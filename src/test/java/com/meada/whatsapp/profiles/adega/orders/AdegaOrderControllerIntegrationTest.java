...
/**
 * Testa os endpoints de pedidos do perfil adega (camada 8.9): list+filter, detail, e o GATE DE
 ...
 * são semeados via jdbcTemplate (no fluxo real vêm da IA — NÃO há POST de criar pedido). O
 * age_confirmed é NOT NULL → sempre true no seed. EvolutionSender real falha o envio (base-url
 * dummy) mas a notificação é best-effort → não quebra a transição. Clone do
 * ComidaOrderControllerIntegrationTest + a ESCAPADA 1.
 */
class AdegaOrderControllerIntegrationTest extends AbstractAdminIntegrationTest {
    ...
    /** Semeia um tenant 'adega' + um pedido 'aguardando' (default da migration), age_confirmed=true. */
    private void seedAdegaOrder() {
        UUID sub = UUID.randomUUID();
        companyId = seedTenantAdmin("adega@test.dev", sub);
        jdbcTemplate.update("update companies set profile_id = 'adega' where id = ?", companyId);
        token = mintValidToken("adega@test.dev", sub);
        ...
        jdbcTemplate.update("insert into contacts (id, company_id, phone_number, name) values (?, ?, ?, ?)",
            contact, companyId, "+5511999990093", "Cliente Pedido");
        jdbcTemplate.update("insert into conversations (id, company_id, contact_id, whatsapp_instance_id, status, handled_by) "
            + "values (?, ?, ?, ?, 'open', 'ai')", conv, companyId, contact, instance);
        UUID menuItem = jdbcTemplate.queryForObject(
            "insert into adega_menu_items (company_id, name, price_cents, category) "
                + "values (?, 'Cerveja', 500, 'cervejas') returning id", UUID.class, companyId);
        // pedido nasce 'aguardando' (default da coluna status). age_confirmed é NOT NULL — sempre true.
        orderId = jdbcTemplate.queryForObject(
            "insert into adega_orders (company_id, conversation_id, contact_id, subtotal_cents, total_cents, delivery_address, age_confirmed) "
                + "values (?, ?, ?, 1000, 1700, 'Rua Y 2', true) returning id", UUID.class, companyId, conv, contact);
        jdbcTemplate.update("insert into adega_order_items (order_id, menu_item_id, qtd, unit_price_cents, item_name_snapshot) "
            + "values (?, ?, 2, 500, 'Cerveja')", orderId, menuItem);
    }
    ...
    void listAndDetail() throws Exception {
        seedAdegaOrder();
        mockMvc.perform(get("/api/adega/orders?status=aguardando").header("Authorization", "Bearer " + token))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.total").value(1))
            .andExpect(jsonPath("$.items[0].status").value("aguardando"));

        mockMvc.perform(get("/api/adega/orders/" + orderId).header("Authorization", "Bearer " + token))
            .andExpect(status().isOk())
    ...
    void patchStatusAccept() throws Exception {
        seedAdegaOrder();
        mockMvc.perform(patch("/api/adega/orders/" + orderId + "/status").header("Authorization", "Bearer " + token)
    ...
    @Test
    @DisplayName("tenant NÃO-adega (legal) batendo no /api/adega/orders → 403 forbidden_wrong_profile")
    void wrongProfile() throws Exception {
        ...
        mockMvc.perform(get("/api/adega/orders").header("Authorization", "Bearer " + t))
