...
/**
 * Testa o shape COMPLETO do MeResponse (email, role, companyId) para os 2 papéis. O
 * filtro em si (auth, reasons) é coberto pelo JwtAuthenticationFilterIntegrationTest;
 * aqui o foco é o controller mapeando corretamente o AuthenticatedUser → MeResponse.
 *
...
class MeControllerIntegrationTest extends AbstractAdminIntegrationTest {
    ...
    @Test
    @DisplayName("super-admin → email, role=super_admin, companyId=null (explícito)")
    ...
    @Test
    @DisplayName("tenant-admin → email, role=tenant_admin, companyId=<uuid do seed>")
